...
def upload_file():
    ...
    try:
        ...
        # ✅ MEMORY-STREAM für große Parquet-Dateien verwenden
        file_stream = io.BytesIO()
        file.save(file_stream)
        file_stream.seek(0)
        
        # ✅ DIREKTES Laden aus Memory-Stream
        try:
            if filename.lower().endswith('.parquet'):
                ...
                file_stream.seek(0)  # Reset für Text-Reading
            ...
            return jsonify({'error': f'Datei konnte nicht geladen werden: {str(load_error)}'}), 400
        ...
        # ✅ OPTIONAL: Backup-Speicherung für große Dateien
        upload_dir = PATHS['uploads']
        upload_dir.mkdir(exist_ok=True)
        filepath = upload_dir / f"{datetime.now().timestamp()}_{filename}"
        
        # Nur bei kleineren Dateien speichern (<50MB)
        if len(df) < 1000000:  # ~50MB bei typischen OHLCV Daten
            if filename.lower().endswith('.parquet'):
            ...
            # Bei großen Dateien: nur Memory-Verarbeitung
        ...
        sessions[session_id] = {
            ...
            'data': df if file_path_str.startswith('memory://') else None,  # ✅ Memory-Cache für große Dateien
            'date_range': [
