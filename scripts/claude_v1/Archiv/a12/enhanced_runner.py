...
✅ KORRIGIERT: Excel Export + Outputs Ordner + Clean Encoding
"""
...
OUTPUTS_DIR = PROJECT_DIR / "outputs" / "divergence_analysis"
OUTPUTS_DIR.mkdir(parents=True, exist_ok=True)

print(f"📁 Output-Verzeichnis: {OUTPUTS_DIR}")
...
def fix_all_datetime_columns_for_excel(df):
    ...
    """
    df_fixed = df.copy()
    logger.info("🔧 Repariere datetime-Spalten für Excel...")
        ...
        try:
            ...
            try:
                if 'date' in col.lower():
                    ...
                    fixed_cols.append(f"{col} (as string)")
            except:
                pass
    ...
    logger.info(f"✅ {len(fixed_cols)} datetime-Spalten repariert: {', '.join(fixed_cols[:5])}{'...' if len(fixed_cols) > 5 else ''}")
    return df_fixed
...
def run_enhanced_analysis():
    ...
    try:
        ...
        # Schritt 8: KUGELSICHERER Excel Export im OUTPUTS-Ordner
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        excel_filename = OUTPUTS_DIR / f"enhanced_divergence_analysis_{timestamp}.xlsx"
        
        logger.info(f"📋 Exportiere Ergebnisse: {excel_filename}")
        
        try:
            # KUGELSICHERER Excel Export mit aggressivem datetime-fix
            ...
            with pd.ExcelWriter(excel_filename, engine='openpyxl') as writer:
                ...
                summary_data.append(['', ''])
                summary_data.append(['OUTPUT INFO', ''])
                summary_data.append(['Excel Location', str(excel_filename)])
                summary_data.append(['Analysis Date', datetime.now().strftime('%Y-%m-%d %H:%M:%S')])
                
                summary_df = pd.DataFrame(summary_data, columns=['Metric', 'Value'])
                summary_df.to_excel(writer, sheet_name='Summary', index=False)
                
                # Alle Divergenzen mit datetime-fix
                all_divergences = []
