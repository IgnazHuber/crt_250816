...
try:
    ...
    from enhanced_divergence_system import Enhanced_Divergence_Analysis, create_enhanced_chart_with_all_divergences
...
class CompleteAnalysisRunner:
    ...
    def run_validation_analysis(self) -> bool:
        ...
        try:
            ...
            logger.info("🔍 Starte Validierung und Backtesting...")
            
            validator = DivergenceValidator(self.df)
            ...
            if validation_results:
                ...
                # Excel Export
                excel_file = validator.export_results_to_excel()
                logger.info(f"📊 Validierungsergebnisse exportiert: {excel_file}")
                
                # Performance Chart erstellen
                validation_chart = validator.create_performance_report()
                validation_chart.show()
                
                logger.info("✅ Validierung abgeschlossen")
                return True
            else:
                logger.warning("⚠️ Keine Validierungsergebnisse erhalten")
                return False
    ...
    def run_parameter_optimization(self) -> bool:
        ...
        try:
            ...
            # SensitivityAnalyzer mit Basis-Daten (ohne Divergenzen)
            base_df = self.original_df.copy()
            Initialize_RSI_EMA_MACD(base_df)
            Local_Max_Min(base_df)
            
            analyzer = SensitivityAnalyzer(base_df)
            
            # Parameter-Bereiche setzen
            analyzer.set_parameter_ranges(self.config['optimization']['parameter_ranges'])
            
            # Vollständige Optimierung durchführen
            optimization_results = analyzer.run_complete_optimization(
            ...
            if optimization_results:
                ...
                # Visualisierung
                sensitivity_chart = analyzer.create_sensitivity_plots()
                sensitivity_chart.show()
                
                # Excel Export
                excel_file = analyzer.export_optimization_results()
                logger.info(f"📊 Optimierungsergebnisse exportiert: {excel_file}")
                
                # Beste Parameter für weitere Analyse verwenden
                if 'optimal' in optimization_results and 'performance_score' in optimization_results['optimal']:
                    best_params = optimization_results['optimal']['performance_score']
                    self.config['default_params'].update({
                        'window': best_params['window'],
                        'candle_tolerance': best_params['candle_tolerance'],
                        'macd_tolerance': best_params['macd_tolerance']
                    })
                    logger.info(f"🎯 Verwende optimierte Parameter: {self.config['default_params']}")
                
                logger.info("✅ Parameter-Optimierung abgeschlossen")
                return True
            else:
                logger.warning("⚠️ Keine Optimierungsergebnisse erhalten")
                return False
    ...
    def run_enhanced_divergence_analysis(self) -> bool:
        ...
        try:
            ...
            # Interaktives Chart mit allen Divergenzen
            enhanced_chart = create_enhanced_chart_with_all_divergences(
                self.df, 
                title="Komplette Divergenz-Analyse - Alle Typen"
            )
            enhanced_chart.show()
