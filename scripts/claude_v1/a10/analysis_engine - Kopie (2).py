...
class AnalysisEngine:
    ...
    def _explain_classic_divergence(self, validation_data, signal_row):
        """Erklärt eine Classic Bullish Divergence"""
        try:
            # Finde die letzten zwei Preis-Minima
            ...
            if len(price_minima) >= 2 and len(rsi_minima) >= 2:
                ...
                price_trend = "fallend" if last_price_min['low'] < prev_price_min['low'] else "steigend"
                rsi_trend = "steigend" if last_rsi_min['rsi'] > prev_rsi_min['rsi'] else "fallend"
                
                explanation = f"""
                CLASSIC BULLISH DIVERGENCE:
                
                📉 Preis-Bewegung: {price_trend}
                  • Vorheriges Minimum: {prev_price_min['low']:.4f} am {prev_price_min['date'][:10]}
                  • Aktuelles Minimum: {last_price_min['low']:.4f} am {last_price_min['date'][:10]}
                  • Differenz: {last_price_min['low'] - prev_price_min['low']:.4f}
                
                📈 RSI-Bewegung: {rsi_trend}
                  • Vorheriges RSI-Min: {prev_rsi_min['rsi']:.2f}
                  • Aktuelles RSI-Min: {last_rsi_min['rsi']:.2f}
                  • Differenz: {last_rsi_min['rsi'] - prev_rsi_min['rsi']:.2f}
                
                ✅ Divergenz bestätigt: Preis macht tieferes Tief, RSI macht höheres Tief
                """
                
                return explanation.strip()
            
            return "Classic Divergence erkannt, aber unvollständige Daten für Erklärung"
            
        except Exception as e:
            return f"Fehler bei Erklärung: {e}"
    
    def _explain_hidden_divergence(self, validation_data, signal_row):
        """Erklärt eine Hidden Bullish Divergence"""
        try:
            # Ähnliche Logik wie Classic, aber für Hidden
            ...
            if len(macd_minima) >= 2:
                ...
                macd_trend = "steigend" if last_macd_min['macd'] > prev_macd_min['macd'] else "fallend"
                
                explanation = f"""
                HIDDEN BULLISH DIVERGENCE:
                
                📊 MACD-Bewegung: {macd_trend}
                  • Vorheriges MACD-Min: {prev_macd_min['macd']:.4f}
                  • Aktuelles MACD-Min: {last_macd_min['macd']:.4f}
                  • Differenz: {last_macd_min['macd'] - prev_macd_min['macd']:.4f}
                
                ✅ Hidden Divergence: MACD zeigt Stärke-Zunahme
                """
                
                return explanation.strip()
            
            return "Hidden Divergence erkannt, aber unvollständige MACD-Daten"
