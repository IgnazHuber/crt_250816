...
function updateCharts(chartData, results, performance, variants) {
    ...
    // EMA-Traces
    ['ema20', 'ema50', 'ema100', 'ema200'].forEach((ema, index) => {
        if (chartData[ema]) {
            traces.push({
                x: chartData.dates,
                y: chartData[ema],
            ...
            if (result[divType]) {
                result[divType].forEach(div => {
                    traces.push({
                        ...
                        type: 'scatter',
                        mode: 'markers',
                        name: `${variant.name} ${divType}`,
                        marker: {
                            symbol: divType === 'classic' ? 'triangle-up' : 'diamond',
                            size: 12,
                        ...
                        yaxis: 'y',
                        hoverinfo: 'text',
                        text: [`${variant.name} ${divType}<br>Date: ${div.date}<br>Low: ${div.low.toFixed(2)}<br>RSI: ${div.rsi.toFixed(2)}<br>MACD: ${div.macd.toFixed(2)}<br>Strength: ${div.validation?.strength_score?.toFixed(2) || 'N/A'}`]
                    });
                });
            }
        });
            ...
            if (result[divType]) {
                result[divType].forEach(div => {
                    validationHtml += `<div class="validation-item">
                        ...
                        <p>Validation: ${div.validation_result?.status || 'N/A'} (${div.validation_result?.message || 'N/A'})</p>
                    </div>`;
                });
            }
        });
