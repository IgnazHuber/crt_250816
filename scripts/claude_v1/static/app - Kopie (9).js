...
// Color palette for variants - green, blue, yellow, orange, then others
const presetColors = { 'basis': 'green' };
const colors = ['green', '#0000FF', '#FFFF00', '#FFA500', '#FF6B6B', '#4ECDC4', '#45B7D1', '#96CEB4', '#FFEAA7', '#DDA0DD', '#98D8C8', '#F7DC6F'];
...
function addVariant() {
    ...
    const variant = {
        ...
        candleTol: candleTol,
        macdTol: macdTol,
        color: presetColors[name] || colors[(variantIdCounter - 2) % colors.length],
        showClassic: true,
        showHidden: true
    };
...
function plotChart(chartData, results) {
    ...
    console.log("🔍 Basis variant:", basisVariant, "Basis results:", basisResults);

    variants.forEach(variant => {
        const res = results[variant.id] || { classic: [], hidden: [] };
        const legendgroup = `variant_${variant.id}`;
        console.log(`🔍 Processing variant ${variant.name}:`, { classic: res.classic.length, hidden: res.hidden.length });
        ...
        if (basisVariant && variant.id !== basisVariant.id) {
            ...
            console.log(`🔍 Additional markers for ${variant.name}:`, { classic: additionalClassic.length, hidden: additionalHidden.length });
            if (additionalClassic.length > 0 || additionalHidden.length > 0) {
                traces.push({
                    x: [...additionalClassic.map(d => d.date), ...additionalHidden.map(d => d.date)],
                    y: [...additionalClassic.map(d => d.low * 1.02), ...additionalHidden.map(d => d.low * 1.02)],
                    mode: 'markers',
                    name: `${variant.name} Additional`,
                    marker: { symbol: 'arrow-up', size: 8, color: variant.color },
                    hoverinfo: 'text',
                    hovertext: [...additionalClassic.map(d => `Neue Divergenz ${d.div_id}: ${d.type}, Stärke: ${d.strength.toFixed(2)}`), ...additionalHidden.map(d => `Neue Divergenz ${d.div_id}: ${d.type}, Stärke: ${d.strength.toFixed(2)}`)],
                    xaxis: 'x',
                    yaxis: 'y',
                    legendgroup: legendgroup,
                    showlegend: true
            ...
            console.log(`🔍 Missing markers for ${variant.name}:`, { classic: missingClassic.length, hidden: missingHidden.length });
            if (missingClassic.length > 0 || missingHidden.length > 0) {
                traces.push({
                    x: [...missingClassic.map(d => d.date), ...missingHidden.map(d => d.date)],
                    y: [...missingClassic.map(d => d.low * 0.98), ...missingHidden.map(d => d.low * 0.98)],
                    mode: 'markers',
                    name: `${variant.name} Missing`,
                    marker: { symbol: 'arrow-down', size: 8, color: variant.color },
                    hoverinfo: 'text',
                    hovertext: [...missingClassic.map(d => `Fehlende Divergenz ${d.div_id}: ${d.type}, Stärke: ${d.strength.toFixed(2)}`), ...missingHidden.map(d => `Fehlende Divergenz ${d.div_id}: ${d.type}, Stärke: ${d.strength.toFixed(2)}`)],
                    xaxis: 'x',
                    yaxis: 'y',
                    legendgroup: legendgroup,
                    showlegend: true
    ...
    const layout = {
        ...
        legend: {
            ...
            itemspacing: 2,
            tracewidth: [
                ...traces.map(t => t.line?.width || t.marker?.line?.width || 1)
            ]
        },
        annotations: [{
            text: resultsText,
