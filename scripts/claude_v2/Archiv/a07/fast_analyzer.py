...
class FastAnalyzer:
    ...
    def analyze_fast(self, df: pd.DataFrame, variants: List[Dict]) -> Dict:
        ...
        # Convert to Polars LazyFrame for speed
    ...
    def _create_chart_data_sampled(self, df_pl: pl.LazyFrame) -> Dict:
        ...
        # Add EMAs with expected names
        ema_mapping = {'ema20': 'ema20', 'ema50': 'ema50', 'ema100': 'ema100', 'ema200': 'ema200'}
        for expected_name, col_name in ema_mapping.items():
            if col_name in df_chart.columns:
                chart_data[expected_name] = df_chart[col_name].to_list()
            else:
                chart_data[expected_name] = None
