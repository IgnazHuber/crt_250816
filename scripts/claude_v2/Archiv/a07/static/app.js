        ...
        if (variantIndex > 0 && firstVariantDivergences) {
            ...
            missingDivergences.forEach(missingDiv => {
                ...
                if (dateIndex >= 0) {
                    ...
                    // Determine the legendgroup based on the current variant (blue circles belong to current variant)
                    const isBullish = missingDiv.type === 'bullish';
                    const legendGroup = `${variant.name}_${missingDiv.divType}_${isBullish ? 'bullish' : 'bearish'}`;
