...
function updateVariantsList() {
    ...
    variants.forEach((variant, index) => {
        ...
        div.innerHTML = `
            <span onclick="cycleVariantColor(${index})" style="color: ${variant.color}; font-weight: bold; cursor: pointer; border: 1px solid ${variant.color}; padding: 2px 4px; border-radius: 3px;" title="Click to change color">⬤ ${variant.name}</span>
        ...
        allDivergences.forEach(div => {
            ...
            traces.push({
                ...
                hovertemplate: `<b>${divName}</b><br>` +
                              ...
                              `Confidence: ${getConfidenceLevel(calculateConfidence(div, variant))} (${(calculateConfidence(div, variant) * 100).toFixed(0)}%)<br>` +
                              `<extra></extra>`
            });
