...
function cycleVariantColor(variantIndex) {
    ...
    // Update chart colors without replotting to preserve zoom/scaling
    if (window.currentResults && sessionId) {
        const chartDiv = document.getElementById('mainChart');
        if (chartDiv && chartDiv.data) {
            const colorUpdates = {};
            const textColorUpdates = {};
            const markerColorUpdates = {};
            const lineColorUpdates = {};
            
            chartDiv.data.forEach((trace, index) => {
                if (trace.name && trace.name.includes(variant.name)) {
                    // Update marker colors
                    if (trace.marker && trace.marker.color === oldColor) {
                        markerColorUpdates[index] = variant.color;
                    }
                    // Update text colors
                    if (trace.textfont && trace.textfont.color === oldColor) {
                        textColorUpdates[index] = variant.color;
                    }
                    // Update line colors
                    if (trace.line && trace.line.color === oldColor) {
                        lineColorUpdates[index] = variant.color;
                    }
                }
            });
            
            // Apply color updates
            if (Object.keys(markerColorUpdates).length > 0) {
                const traceIndices = Object.keys(markerColorUpdates).map(Number);
                const newColors = traceIndices.map(i => markerColorUpdates[i]);
                Plotly.restyle(chartDiv, {'marker.color': newColors}, traceIndices);
            }
            if (Object.keys(textColorUpdates).length > 0) {
                const traceIndices = Object.keys(textColorUpdates).map(Number);
                const newColors = traceIndices.map(i => textColorUpdates[i]);
                Plotly.restyle(chartDiv, {'textfont.color': newColors}, traceIndices);
            }
            if (Object.keys(lineColorUpdates).length > 0) {
                const traceIndices = Object.keys(lineColorUpdates).map(Number);
                const newColors = traceIndices.map(i => lineColorUpdates[i]);
                Plotly.restyle(chartDiv, {'line.color': newColors}, traceIndices);
            }
        }
    }
}
...
function calculateSuccessRate(div, chartData, lookAheadDays = 30) {
    ...
    if (dateIndex === -1 || dateIndex >= chartData.dates.length - 2) {
        return { success: null, performance: null, reason: 'No future data' };
    }
    
    // Entry price: next day's opening price (realistic trading scenario)
    const entryPrice = chartData.open[dateIndex + 1];
    
    // Find the highest high and lowest low in the lookAhead period
    const endIndex = Math.min(dateIndex + 1 + lookAheadDays, chartData.dates.length - 1);
    
    if (!entryPrice) {
        return { success: null, performance: null, reason: 'Missing entry price data' };
    }
    
    let maxPrice = entryPrice;
    let minPrice = entryPrice;
    
    // Find max potential gain and max potential loss in the period
    for (let i = dateIndex + 1; i <= endIndex; i++) {
        if (chartData.high[i] > maxPrice) maxPrice = chartData.high[i];
        if (chartData.low[i] < minPrice) minPrice = chartData.low[i];
    }
    
    const maxGain = ((maxPrice - entryPrice) / entryPrice) * 100;
    const maxLoss = ((minPrice - entryPrice) / entryPrice) * 100;
    
    // Determine if bullish or bearish signal
    const isBullish = div.type === 'classic';
    
    // For bullish: show max potential gain, for bearish: show max potential loss
    const relevantPerformance = isBullish ? maxGain : Math.abs(maxLoss);
    const direction = isBullish ? '+' : '-';
    const success = isBullish ? maxGain > 0 : maxLoss < 0;
    ...
    return {
        ...
        lookAheadDays: endIndex - (dateIndex + 1)
    };
}
    ...
    if (index < variants.length) {
        switch(type) {
            ...
            case 'success':
                ...
                break;
            case 'rsi_names':
                variants[index].showRSINames = document.getElementById(`rsi_names_${index}`).checked;
                // Update visibility of RSI name traces without replotting
                if (window.currentResults && sessionId) {
                    updateTextTraceVisibility('rsi_names', index);
                    return;
                }
                break;
            case 'rsi_numbers':
                variants[index].showRSINumbers = document.getElementById(`rsi_numbers_${index}`).checked;
                // Update visibility of RSI number traces without replotting
                if (window.currentResults && sessionId) {
                    updateTextTraceVisibility('rsi_numbers', index);
                    return;
                }
                break;
            case 'macd_names':
                variants[index].showMACDNames = document.getElementById(`macd_names_${index}`).checked;
                // Update visibility of MACD name traces without replotting
                if (window.currentResults && sessionId) {
                    updateTextTraceVisibility('macd_names', index);
                    return;
                }
                break;
            case 'macd_numbers':
                variants[index].showMACDNumbers = document.getElementById(`macd_numbers_${index}`).checked;
                // Update visibility of MACD number traces without replotting
                if (window.currentResults && sessionId) {
                    updateTextTraceVisibility('macd_numbers', index);
                    return;
                }
                break;
        }
...
function updateTextTraceVisibility(type, variantIndex) {
    ...
    const isVisible = variant[type === 'names' ? 'showNames' : 
                             type === 'numbers' ? 'showNumbers' : 
                             type === 'circles' ? 'showCircles' :
                             type === 'success' ? 'showSuccessRate' :
                             type === 'rsi_names' ? 'showRSINames' :
                             type === 'rsi_numbers' ? 'showRSINumbers' :
                             type === 'macd_names' ? 'showMACDNames' :
                             type === 'macd_numbers' ? 'showMACDNumbers' : 'showCircles'];
    ...
    chartDiv.data.forEach((trace, traceIndex) => {
        ...
        } else if (type === 'success' && (traceName.includes('Success Rate') || traceName.includes('Success'))) {
            // Success rate traces belong to the variant that creates them
            matchesType = traceName.includes(variantName);
        } else if (type === 'rsi_names' && traceName.includes('RSI Label')) {
            // RSI names
            matchesType = traceName.includes(variantName);
        } else if (type === 'rsi_numbers' && traceName.includes('RSI Number')) {
            // RSI numbers
            matchesType = traceName.includes(variantName);
        } else if (type === 'macd_names' && traceName.includes('MACD Label')) {
            // MACD names
            matchesType = traceName.includes(variantName);
        } else if (type === 'macd_numbers' && traceName.includes('MACD Number')) {
            // MACD numbers
            matchesType = traceName.includes(variantName);
        }
...
function plotChart(chartData, results) {
        ...
        allDivergences.forEach(div => {
            ...
            traces.push({
                ...
                hovertemplate: (() => {
                    ...
                    const successData = calculateSuccessRate(div, chartData);
                    const successText = successData.success !== null ? 
                        `Success: ${successData.direction}${successData.performance.toFixed(1)}% (${successData.lookAheadDays}d)` : 
                        'Success: No future data';
            ...
            if (variant.showNumbers) {
                ...
                traces.push({
                x: [div.date],
                y: [div.low - (div.low * 0.008)], // Position closer to marker
                mode: 'text',
                type: 'scatter',
                name: `${variant.name} ${divName} Number`,
                text: [globalDivCount],
                textposition: 'bottom center',
                textfont: { color: variant.color, size: 15, family: 'Helvetica, Arial, sans-serif', weight: 'bold' },
                xaxis: 'x',
                yaxis: 'y',
                legendgroup: `${variant.name}_${div.divType}_${isBullish ? 'bullish' : 'bearish'}`,
                showlegend: false,
            ...
            if (variant.showSuccessRate) {
                ...
                if (successData.success !== null) {
                    ...
                    traces.push({
                        x: [div.date],
                        y: [div.low - (div.low * 0.018)], // Position below numbers
                        mode: 'text',
                        type: 'scatter',
                        text: [successText],
                        textfont: { color: successColor, size: 15, family: 'Helvetica, Arial, sans-serif', weight: 'bold' },
                        textposition: 'bottom center',
            ...
            // Calculate RSI offset for consistent positioning
            ...
            // Add RSI number with staggering (if enabled)
            if (variant.showRSINumbers) {
            traces.push({
                x: [div.date],
                y: [div.rsi - 5 - rsiOffset],
                mode: 'text',
                type: 'scatter',
            ...
            // Calculate MACD offset for consistent positioning
            ...
            // Add MACD number with staggering (further below marker, if enabled)
            if (variant.showMACDNumbers) {
            const macdNumberY = div.macd < 0 ? div.macd + 0.008 + macdOffset : div.macd - 0.008 - macdOffset;
            traces.push({
                x: [div.date],
                y: [macdNumberY],
                mode: 'text',
            ...
            // Add MACD spray line (shorter)
            const macdSprayEndY = div.macd < 0 ? div.macd + 0.001 + macdOffset : div.macd - 0.001 - macdOffset;
            traces.push({
                x: [div.date, div.date],
                y: [div.macd, macdSprayEndY],
                mode: 'lines',
                type: 'scatter',
            ...
            if (dateIndex > variant.window && shouldShowDetails) {
                ...
                traces.push({
                    ...
                    line: { color: variant.color, width: 2.25, dash: 'dash', shape: 'spline' },
                    xaxis: 'x',
                    yaxis: 'y',
                    legendgroup: `${variant.name}_${div.divType}_${isBullish ? 'bullish' : 'bearish'}`,
                    showlegend: false,
                ...
                if (variant.showNames) {
                    ...
                    traces.push({
                    x: [div.date],
                    y: [div.low - (div.low * 0.025)], // Position below marker
                    mode: 'text',
                    type: 'scatter',
                    ...
                    // Add success rate under the name in candlestick chart
                    const successData = calculateSuccessRate(div, chartData);
                    if (successData.success !== null) {
                        const successText = `${successData.direction}${successData.performance.toFixed(1)}%`;
                        const successColor = successData.success ? '#00FF00' : '#FF0000';
                        
                        traces.push({
                            x: [div.date],
                            y: [div.low - (div.low * 0.035)], // Position below divergence name
                            mode: 'text',
                            type: 'scatter',
                            name: `${variant.name} ${divName} Success`,
                            text: [successText],
                            textposition: 'bottom center',
                            textfont: { color: successColor, size: 15, family: 'Helvetica, Arial, sans-serif', weight: 'bold' },
                            xaxis: 'x',
                ...
                // Add divergence name annotation to RSI plot (if enabled)
                if (variant.showRSINames) {
                traces.push({
                    x: [div.date],
                    y: [div.rsi - 8 - rsiOffset],
                    mode: 'text',
                    type: 'scatter',
                    name: `${variant.name} ${divName} RSI Label`,
                    text: [divName],
                    textposition: 'bottom center',
                    textfont: { color: variant.color, size: 17, family: 'Arial Bold' },
                    xaxis: 'x',
                    yaxis: 'y2',
                    legendgroup: `${variant.name}_${div.divType}_${isBullish ? 'bullish' : 'bearish'}`,
                    showlegend: false,
                ...
                // Add success rate under the name in RSI chart
                const successDataRSI = calculateSuccessRate(div, chartData);
                if (successDataRSI.success !== null) {
                    const successText = `${successDataRSI.direction}${successDataRSI.performance.toFixed(1)}%`;
                    const successColor = successDataRSI.success ? '#00FF00' : '#FF0000';
                    
                    traces.push({
                        x: [div.date],
                        y: [div.rsi - 16 - rsiOffset], // Position much further below divergence name
                        mode: 'text',
                        type: 'scatter',
                        name: `${variant.name} ${divName} RSI Success`,
                        text: [successText],
                        textposition: 'bottom center',
                        textfont: { color: successColor, size: 15, family: 'Helvetica, Arial, sans-serif', weight: 'bold' },
                        xaxis: 'x',
                ...
                // Add divergence name annotation to MACD plot (much further below, if enabled)
                if (variant.showMACDNames) {
                traces.push({
                    x: [div.date],
                    y: [div.macd < 0 ? div.macd + 0.015 + macdOffset : div.macd - 0.015 - macdOffset],
                    mode: 'text',
                    type: 'scatter',
                ...
                // Add success rate under the name in MACD chart
                const successDataMACD = calculateSuccessRate(div, chartData);
                if (successDataMACD.success !== null) {
                    const successText = `${successDataMACD.direction}${successDataMACD.performance.toFixed(1)}%`;
                    const successColor = successDataMACD.success ? '#00FF00' : '#FF0000';
                    
                    traces.push({
                        x: [div.date],
                        y: [div.macd < 0 ? div.macd + 0.030 + macdOffset : div.macd - 0.030 - macdOffset], // Position much further below divergence name
                        mode: 'text',
                        type: 'scatter',
                        name: `${variant.name} ${divName} MACD Success`,
                        text: [successText],
                        textposition: 'bottom center',
                        textfont: { color: successColor, size: 15, family: 'Helvetica, Arial, sans-serif', weight: 'bold' },
                        xaxis: 'x',
    ...
    const layout = {
        ...
        yaxis: {
            title: 'Price',
            domain: [0.47, 1], // Restored vertical spacing
            gridcolor: 'rgba(255,255,255,0.1)',
            titlefont: { color: '#FFFFFF', size: 8 },
            tickfont: { color: '#FFFFFF', size: 8 },
            fixedrange: false,
        ...
        yaxis2: {
            title: 'RSI',
            domain: [0.25, 0.45], // Restored vertical spacing
            anchor: 'x',
            gridcolor: 'rgba(255,255,255,0.1)',
            titlefont: { color: '#FFFFFF', size: 8 },
            tickfont: { color: '#FFFFFF', size: 8 },
        ...
        yaxis3: {
            ...
            domain: [0, 0.24], // Restored vertical spacing
            anchor: 'x',
            gridcolor: 'rgba(255,255,255,0.1)',
            titlefont: { color: '#FFFFFF', size: 8 },
            tickfont: { color: '#FFFFFF', size: 8 },
    ...
    try {
        ...
        // Use Plotly.react to preserve zoom/pan - never reset plots automatically
        if (chartDiv && chartDiv.data && chartDiv.data.length > 0) {
            // Chart exists, update without resetting
...
function updateStats(results) {
    ...
    if (window.divergenceDetails && window.divergenceDetails.length > 0) {
        ...
        window.divergenceDetails.forEach(div => {
            const typeColor = div.type.includes('bull') ? '#00FF00' : '#FF0000'; // Green for bullish, red for bearish
            html += `<tr style="border-bottom: 1px solid rgba(255,255,255,0.05); cursor: pointer; transition: background-color 0.2s;" 
                ...
                <td style="text-align: center; padding: 6px; font-weight: bold; color: #FFFFFF;">${div.id}</td>
                <td style="padding: 6px;">${div.variant}</td>
                <td style="padding: 6px; color: ${typeColor}; font-weight: bold;">${div.type}</td>
