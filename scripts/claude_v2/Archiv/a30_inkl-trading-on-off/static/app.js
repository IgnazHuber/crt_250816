...
function plotChart(chartData, results) {
    ...
    variants.forEach((variant, variantIndex) => {
        ...
        allDivergences.forEach(div => {
            ...
            traces.push({
                x: [div.date],
                y: [div.rsi - (div.rsi * 0.05)], // Numbers adequately below RSI marker
                mode: 'text',
                type: 'scatter',
            ...
            traces.push({
                x: [div.date],
                y: [div.macd - (Math.abs(div.macd) * 0.1)], // Numbers adequately below MACD marker
                mode: 'text',
                type: 'scatter',
            ...
            // Add MACD spray line
            const macdNumberY = div.macd - (Math.abs(div.macd) * 0.1);
            traces.push({
                x: [div.date, div.date],
            ...
            if (dateIndex > variant.window && shouldShowDetails) {
                ...
                traces.push({
                    x: [div.date],
                    y: [div.rsi - (div.rsi * 0.1)], // Names adequately below RSI numbers
                    mode: 'text',
                    type: 'scatter',
                ...
                traces.push({
                    x: [div.date],
                    y: [div.macd - (Math.abs(div.macd) * 0.2)], // Names adequately below MACD numbers
                    mode: 'text',
                    type: 'scatter',
    ...
    if (tradingEnabled) {
        ...
        // Run trading simulation for this variant
        if (variantBullishSignals.length > 0) {
            const tradingResult = simulateBullishTradingStrategy(chartData, variantBullishSignals, 10000);
...
function updateStats(results) {
    ...
    html += '<tr style="border-bottom: 2px solid rgba(255,255,255,0.2);"><th style="text-align: left; padding: 8px; font-size: 13px;">Variant</th><th style="text-align: center; padding: 8px; font-size: 13px;">Classic</th><th style="text-align: center; padding: 8px; font-size: 13px;">Hidden</th><th style="text-align: center; padding: 8px; font-size: 13px;">Total</th><th style="text-align: center; padding: 8px; font-size: 13px;">Capital</th></tr>';
    ...
    variants.forEach(v => {
        ...
        const capitalDisplay = tradingRes ? 
            `<span style="color: ${tradingRes.totalReturn >= 0 ? '#00FF00' : '#FF0000'};">$${Math.round(tradingRes.finalCapital).toLocaleString()} (${tradingRes.totalReturn >= 0 ? '+' : ''}${tradingRes.totalReturn.toFixed(1)}%)</span>` : 
