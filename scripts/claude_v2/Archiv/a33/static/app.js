...
function updateVariantsList() {
    ...
    variants.forEach((variant, index) => {
        ...
        div.innerHTML = `
            ...
            <span>W:${variant.window} C:${variant.candleTol}% M:${variant.macdTol}% | SL:${variant.stopLoss || 5}% TP:${variant.takeProfit || 25}% Risk:${variant.positionSize || 100}% | SD:${variant.startDate || '-'} ED:${variant.endDate || '-'}</span>
...
function plotChart(chartData, results) {
    ...
    variants.forEach((variant, variantIndex) => {
        ...
        allDivergences.forEach(div => {
            ...
            if (variant.showNumbers) {
                traces.push({
                x: [div.date],
                y: [div.low - (div.low * 0.02)], // Numbers adequately below marker
                mode: 'text',
                type: 'scatter',
                name: `${variant.name} ${divName} Number`,
                text: [globalDivCount],
                textposition: 'middle center',
                textfont: { color: variant.color, size: 10, family: 'Helvetica, Arial, sans-serif', weight: 'bold' },
                xaxis: 'x',
                yaxis: 'y',
                legendgroup: `${variant.name}_${div.divType}_${isBullish ? 'bullish' : 'bearish'}`,
                showlegend: false,
            ...
            if (variant.showSuccessRate) {
                ...
                if (successData.success !== null) {
                    ...
                    traces.push({
                        x: [div.date],
                        y: [div.low - (div.low * 0.06)], // Success adequately below names
                        mode: 'text',
                        type: 'scatter',
                        text: [successText],
                        textfont: { color: successColor, size: 10, family: 'Helvetica, Arial, sans-serif', weight: 'bold' },
            ...
            traces.push({
                x: [div.date],
                y: [div.rsi - (rsiRange * 0.02)], // Numbers adequately below RSI marker
                mode: 'text',
                type: 'scatter',
            ...
            traces.push({
                x: [div.date],
                y: [div.macd - (macdRange * 0.02)], // Numbers adequately below MACD marker
                mode: 'text',
                type: 'scatter',
            ...
            if (dateIndex > variant.window && shouldShowDetails) {
                ...
                traces.push({
                    ...
                    line: { color: variant.color, width: 1.5, dash: 'dash', shape: 'spline' },
                    xaxis: 'x',
                    yaxis: 'y',
                    legendgroup: `${variant.name}_${div.divType}_${isBullish ? 'bullish' : 'bearish'}`,
                    showlegend: false,
                ...
                if (variant.showNames) {
                    traces.push({
                    x: [div.date],
                    y: [div.low - (div.low * 0.04)], // Names adequately below numbers
                    mode: 'text',
                    type: 'scatter',
                ...
                traces.push({
                    x: [div.date],
                    y: [div.rsi - (rsiRange * 0.04)], // Names adequately below RSI numbers
                    mode: 'text',
                    type: 'scatter',
                ...
                traces.push({
                    x: [div.date],
                    y: [div.macd - (macdRange * 0.04)], // Names adequately below MACD numbers
                    mode: 'text',
                    type: 'scatter',
    ...
    if (tradingEnabled) {
        ...
        // Use variant-specific trading parameters
        const stopLossPercent = variant.stopLoss || 5;
        const takeProfitPercent = variant.takeProfit || 25;
        const positionSizePercent = variant.positionSize || 100;
        const startDate = variant.startDate || null;
        const endDate = variant.endDate || null;
        ...
        if (variantBullishSignals.length > 0) {
            const tradingResult = simulateBullishTradingStrategy(chartData, variantBullishSignals, 10000, stopLossPercent, takeProfitPercent, positionSizePercent, startDate, endDate);
