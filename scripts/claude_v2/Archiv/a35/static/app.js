...
function plotChart(chartData, results) {
    ...
    try {
        ...
        if (chartDiv && chartDiv.layout) {
            preservedRanges = {
                xaxis: chartDiv.layout.xaxis ? { range: chartDiv.layout.xaxis.range } : null,
                yaxis: chartDiv.layout.yaxis ? { range: chartDiv.layout.yaxis.range } : null,
                yaxis2: chartDiv.layout.yaxis2 ? { range: chartDiv.layout.yaxis2.range } : null,
                yaxis3: chartDiv.layout.yaxis3 ? { range: chartDiv.layout.yaxis3.range } : null,
                yaxis4: chartDiv.layout.yaxis4 ? { range: chartDiv.layout.yaxis4.range } : null
            };
        }
        ...
        if (preservedRanges) {
            ...
            if (Object.keys(rangeUpdate).length > 0) {
                setTimeout(() => {
                    Plotly.relayout('mainChart', rangeUpdate);
