...
def calculate_rsi_vectorized(prices: np.ndarray, period: int = RSI_PERIOD) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    # Apply fast vectorized rounding for performance (JIT-compatible)
    changes_rounded = np.round(changes, STORAGE_PRECISION['rsi_components'])
    gains_rounded = np.round(gains, STORAGE_PRECISION['rsi_components']) 
    losses_rounded = np.round(losses, STORAGE_PRECISION['rsi_components'])
    rsi_final = np.round(rsi_values, STORAGE_PRECISION['rsi_final'])
