...
# Wichtig: Wir verwenden die bestehende Flask-App aus server_numba.py (ultra-fast version)
try:
    from server_numba import app  # server_numba.py must provide Flask instance `app`
except ImportError as e:
    # Fallback to original server if Numba version not available
    try:
        from server import app  # server.py must provide Flask instance `app`
        print("Warning: Using original server.py instead of optimized server_numba.py", file=sys.stderr)
    except ImportError as e2:
        print(f"Could not import server_numba.app or server.app: {e2}", file=sys.stderr)
        print("Make sure server_numba.py or server.py exists and exports 'app'", file=sys.stderr)
        sys.exit(1)
except Exception as e:
    print(f"Unexpected error importing server_numba.app: {e}", file=sys.stderr)
    raise
...
def check_dependencies() -> None:
    ...
    required_files: List[str] = [
        "server_numba.py",  # Ultra-fast Numba-optimized server
        "data_processor_v2.py",  # Fast loading module
        "Initialize_RSI_EMA_MACD_numba.py",  # Numba-optimized version
        "Local_Maximas_Minimas_numba.py",   # Numba-optimized version
        "CBullDivg_Analysis_numba.py",      # Numba-optimized version
    ]
    ...
    logger.info("============================================================")
    logger.info("🚀 BULLISH DIVERGENCE ANALYZER - Ultra-Fast Numba Edition")
    logger.info("============================================================")
    ...
    logger.info("    • Ultra-fast Numba JIT compilation (50-100x faster)")
    logger.info("    • LLVM-optimized technical indicators")
    logger.info("    • JIT-compiled local maxima/minima detection")  
    logger.info("    • High-performance divergence analysis")
    logger.info("    • WebGL-accelerated chart rendering")
    logger.info("    • Fast Polars-based data loading (2-10x faster)")
    logger.info("    • File explorer upload workflow (CSV/Parquet)")
