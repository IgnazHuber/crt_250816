...
function sampleChartData(chartData, maxPoints = 8000) {
    if (!chartData.dates || chartData.dates.length <= maxPoints) {
        return chartData;
    }
    
    console.log(`⚡ Large dataset (${chartData.dates.length.toLocaleString()} points) - applying intelligent sampling to ${maxPoints.toLocaleString()} points`);
    ...
    try {
        ...
        if (dataSize > 10000) {
            ...
            const optimizedTraces = traces.map(trace => {
                ...
                if (trace.type === 'scatter' && (trace.mode === 'lines' || trace.mode === 'markers') && dataSize > 15000) {
                    return {
                        ...trace,
                        type: 'scattergl',
                        // Disable hover for better performance on very large datasets
                        hoverinfo: dataSize > 50000 ? 'skip' : trace.hoverinfo
                    };
                }
