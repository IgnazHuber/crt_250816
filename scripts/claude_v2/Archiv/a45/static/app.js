...
function cycleVariantColor(variantIndex) {
    ...
    // Update trace colors without replotting
    if (window.currentResults && sessionId) {
        updateTraceColors(variantIndex, variant.color);
    }
}

function updateTraceColors(variantIndex, newColor) {
    ...
    chartDiv.data.forEach((trace, traceIndex) => {
        ...
        // Check if this trace belongs to this variant
        if (traceName.includes(variantName)) {
            const update = {};
            ...
            // Update text colors
            if (trace.textfont && trace.textfont.color) {
                update['textfont.color'] = newColor;
            }
    ...
    // Apply all color updates in one batch
    if (updates.length > 0) {
        updates.forEach((update, i) => {
            Plotly.restyle(chartDiv, update, traceIndices[i]);
        });
    }
}
...
function sampleChartData(chartData, maxPoints = 12000) {
    if (!chartData.dates || chartData.dates.length <= maxPoints) {
        return chartData;
    }
    
    console.log(`⚡ Large dataset (${chartData.dates.length.toLocaleString()} points) - applying progressive sampling to ${maxPoints.toLocaleString()} points`);
    
    // For extremely large datasets, use more aggressive sampling initially
    if (chartData.dates.length > 100000) {
        maxPoints = Math.min(8000, maxPoints);
        console.log(`📊 Extremely large dataset - reducing to ${maxPoints.toLocaleString()} points for initial render`);
    }
    ...
    // Copy EMA data if available (using correct naming from backend)
...
function plotChart(chartData, results) {
    ...
    console.log("Plotting chart with data:", { 
        ...
        EMA_20_available: !!sampledData.EMA_20
    });
    ...
    // Add EMAs
    if (sampledData.EMA_20) {
        traces.push({
            x: sampledData.dates,
            y: sampledData.EMA_20,
            type: 'scatter',
            name: 'EMA 20',
            xaxis: 'x',
            yaxis: 'y',
    ...
    if (sampledData.EMA_50) {
        traces.push({
            x: sampledData.dates,
            y: sampledData.EMA_50,
            type: 'scatter',
            name: 'EMA 50',
            xaxis: 'x',
            yaxis: 'y',
    ...
    if (sampledData.EMA_100) {
        traces.push({
            x: sampledData.dates,
            y: sampledData.EMA_100,
            type: 'scatter',
            name: 'EMA 100',
            xaxis: 'x',
            yaxis: 'y',
    ...
    if (sampledData.EMA_200) {
        traces.push({
            x: sampledData.dates,
            y: sampledData.EMA_200,
            type: 'scatter',
            name: 'EMA 200',
            xaxis: 'x',
            yaxis: 'y',
    ...
    variants.forEach((variant, variantIndex) => {
        ...
        allDivergences.forEach(div => {
            ...
            // Mark as new if this divergence is not found in first variant (for yellow highlighting)
            if (variantIndex > 0 && firstVariantDivergences) {
                const existsInFirst = firstVariantDivergences.some(firstDiv => 
                    firstDiv.date === div.date && 
                    firstDiv.divType === div.divType
                );
                div.is_new = !existsInFirst;
            } else {
                div.is_new = false; // First variant items are never "new"
            }
            ...
            // Get capital at this date from trading results
            let capitalAtDate = null;
            if (window.allTradingResults && window.allTradingResults[variant.id]) {
                const tradingResult = window.allTradingResults[variant.id];
                const capitalEntry = tradingResult.capitalHistory.find(h => h.date === div.date);
                capitalAtDate = capitalEntry ? capitalEntry.capital : null;
            }
            ...
            // Add marker number positioned UNDER marker
            if (variant.showNumbers) {
                traces.push({
                x: [div.date],
                y: [div.low * (1 - (variant.numberPosition || 4) / 100)], // Numbers positioned relative to marker
                mode: 'text',
                type: 'scatter',
                name: `${variant.name} ${divName} Number`,
                text: [globalDivCount],
                textposition: 'middle center',
                textfont: { color: variant.color, size: variant.textSize || 10, family: 'Helvetica, Arial, sans-serif', weight: 'bold' },
                xaxis: 'x',
                yaxis: 'y',
                legendgroup: `${variant.name}_${div.divType}_${isBullish ? 'bullish' : 'bearish'}`,
                showlegend: false,
            ...
            if (variant.showSuccessRate) {
                ...
                if (successData.success !== null) {
                    ...
                    traces.push({
                        x: [div.date],
                        y: [div.low * (1 - (variant.successPosition || 8) / 100)], // Success positioned relative to marker
                        mode: 'text',
                        type: 'scatter',
                        text: [successText],
                        textfont: { color: successColor, size: variant.textSize || 10, family: 'Helvetica, Arial, sans-serif', weight: 'bold' },
                        ...
                        legendgroup: `${variant.name}_${div.divType}_${isBullish ? 'bullish' : 'bearish'}`,
                        showlegend: false,
                        hovertemplate: `<b>Success Rate</b><br>` +
                                      'Date: %{x}<br>' +
                                      'Performance: ' + successText + '<br>' +
                                      '<extra></extra>'
                    });
                }
            }
            ...
            traces.push({
                x: [div.date],
                y: [div.rsi - (div.rsi * (variant.numberPosition || 4) / 100)], // Numbers positioned relative to marker
                mode: 'text',
                type: 'scatter',
            ...
            const macdRange = Math.max(...chartData.macd_histogram.filter(x => x !== null && !isNaN(x))) - Math.min(...chartData.macd_histogram.filter(x => x !== null && !isNaN(x)));
            traces.push({
                x: [div.date],
                y: [div.macd - (Math.abs(div.macd) * (variant.numberPosition || 4) / 100)], // Numbers positioned relative to marker
                mode: 'text',
                type: 'scatter',
            ...
            // Add MACD spray line
            const macdNumberY = div.macd - (macdRange * 0.02);
            traces.push({
                x: [div.date, div.date],
            ...
            if (dateIndex > variant.window && shouldShowDetails) {
                ...
                // Create straight arrows for Price plot (minimal curvature)
                traces.push({
                    x: [chartData.dates[startIndex], div.date],
                    y: [chartData.low[startIndex], div.low],
                    mode: 'lines',
                    type: 'scatter',
                    name: `${variant.name} ${divName} Trend`,
                    line: { color: variant.color, width: variant.lineWidth || 1.5, dash: 'dash', shape: 'linear' },
                    xaxis: 'x',
                    yaxis: 'y',
                    legendgroup: `${variant.name}_${div.divType}_${isBullish ? 'bullish' : 'bearish'}`,
                    showlegend: false,
                ...
                // Add divergence name annotation to Price plot - positioned UNDER numbers
                if (variant.showNames) {
                    traces.push({
                    x: [div.date],
                    y: [div.low * (1 - (variant.namePosition || 6) / 100)], // Names positioned relative to marker
                    mode: 'text',
                    type: 'scatter',
                ...
                // Create straight arrows for RSI plot
                traces.push({
                    x: [chartData.dates[startIndex], div.date],
                    y: [chartData.rsi[startIndex], div.rsi],
                    mode: 'lines',
                    type: 'scatter',
                ...
                traces.push({
                    x: [div.date],
                    y: [div.rsi - (div.rsi * (variant.namePosition || 6) / 100)], // Names positioned relative to marker
                    mode: 'text',
                    type: 'scatter',
                ...
                // Create straight arrows for MACD plot
                traces.push({
                    x: [chartData.dates[startIndex], div.date],
                    y: [chartData.macd_histogram[startIndex], div.macd],
                    mode: 'lines',
                    type: 'scatter',
                ...
                traces.push({
                    x: [div.date],
                    y: [div.macd - (Math.abs(div.macd) * (variant.namePosition || 6) / 100)], // Names positioned relative to marker
                    mode: 'text',
                    type: 'scatter',
    ...
    // Run bullish trading strategy simulation for each variant (if enabled)
    const allTradingResults = {};
    const tradingEnabled = document.getElementById('enableTrading').checked;
    ...
    if (tradingEnabled) {
        ...
        // Collect bullish signals for this variant
        const variantBullishSignals = [];
        res.classic.forEach(div => {
            variantBullishSignals.push({
                date: div.date,
                variant: variant.name,
        ...
        // Run trading simulation for this variant with variant-specific parameters
        if (variantBullishSignals.length > 0) {
            const tradingResult = simulateBullishTradingStrategy(
                chartData, 
                variantBullishSignals, 
                10000, 
                variant.stopLoss || 5, 
                variant.takeProfit || 25, 
                variant.positionSize || 100, 
                variant.startDate || null, 
                variant.endDate || null
            );
            allTradingResults[variant.id] = {
                ...tradingResult,
    ...
    // Use the first variant's result for main display (backward compatibility)
    const tradingResult = Object.values(allTradingResults)[0] || null;
    
    // Add capital tracking plots for each variant (if trading enabled)
    if (tradingEnabled) {
        Object.values(allTradingResults).forEach((result, index) => {
        // Find the corresponding variant to get styling parameters
        const variant = variants.find(v => v.name === result.variantName);
        
        traces.push({
            x: result.capitalHistory.map(h => h.date),
            y: result.capitalHistory.map(h => h.capital),
            type: 'scatter',
            mode: 'lines',
            name: `${result.variantName} Capital`,
            xaxis: 'x',
            yaxis: 'y4',
            line: { 
                color: result.color, 
                width: variant?.lineWidth || 2,
                shape: 'linear'
            },
            legendgroup: 'trading',
            showlegend: true,
            hovertemplate: `<b>${result.variantName} Capital</b><br>` +
                          'Date: %{x}<br>' +
                          'Value: $%{y:,.0f}<br>' +
                          '<extra></extra>'
        });
    ...
    if (tradingEnabled && tradingResult && tradingResult.tradeHistory) {
        ...
        // Capital text at entry - only show for each variant individually
        if (Object.keys(allTradingResults).length === 1) {
            traces.push({
                x: [trade.date],
                ...
                textposition: 'middle center',
                textfont: { color: '#00FF00', size: variants[0]?.textSize || 10, family: 'Helvetica, Arial, sans-serif', weight: 'bold' },
                xaxis: 'x',
                yaxis: 'y4',
                legendgroup: 'trading',
                showlegend: false,
        ...
        // Capital text at exit - only show for single variant
        if (Object.keys(allTradingResults).length === 1) {
            traces.push({
                x: [trade.date],
                ...
                text: [`$${Math.round(trade.capital).toLocaleString()}`],
                textposition: 'middle center',
                textfont: { color: markerColor, size: variants[0]?.textSize || 10, family: 'Helvetica, Arial, sans-serif', weight: 'bold' },
                xaxis: 'x',
                yaxis: 'y4',
                legendgroup: 'trading',
                showlegend: false,
    ...
    window.allTradingResults = allTradingResults;
    ...
    variants.forEach(variant => {
        ...
        if (variantDivs.length > 0) {
            ...
            // Get capital result for this variant
            const capitalResult = allTradingResults[variant.id];
            const capitalInfo = capitalResult ? 
                `$${Math.round(capitalResult.finalCapital).toLocaleString()} (${capitalResult.totalReturn >= 0 ? '+' : ''}${capitalResult.totalReturn.toFixed(1)}%)` : 
                'No trades';
            ...
            resultsSummary += `  Capital: <span style="color: ${capitalResult && capitalResult.totalReturn >= 0 ? '#00FF00' : '#FF0000'};">${capitalInfo}</span><br>`;
        }
    });
    ...
    const layout = {
        ...
        yaxis: {
            title: 'Price',
            domain: [0.55, 1], // Reduced price plot to make room for taller RSI/MACD
            gridcolor: 'rgba(255,255,255,0.1)',
            titlefont: { color: '#FFFFFF', size: 8 },
            tickfont: { color: '#FFFFFF', size: 8 },
            fixedrange: false,
        ...
        yaxis2: {
            title: 'RSI',
            domain: [0.35, 0.53], // Increased RSI height by 50% (13% → 18%)
            anchor: 'x',
            gridcolor: 'rgba(255,255,255,0.1)',
            titlefont: { color: '#FFFFFF', size: 8 },
            tickfont: { color: '#FFFFFF', size: 8 },
        ...
        yaxis3: {
            ...
            domain: [0.17, 0.33], // Increased MACD height by 50% (13% → 16%)
            anchor: 'x',
            gridcolor: 'rgba(255,255,255,0.1)',
            titlefont: { color: '#FFFFFF', size: 8 },
            tickfont: { color: '#FFFFFF', size: 8 },
        ...
        showlegend: true,
        legend: {
            orientation: 'v', // Vertical legend for better visibility
            y: 1,
            x: 1.02, // Position to the right of chart
            bgcolor: 'rgba(0,0,0,0.8)',
            font: { color: '#FFFFFF', size: 9 }, // Slightly larger font
        ...
        annotations: [
            {
                ...
                yref: 'paper',
                x: 0.02,
                y: 0.95, // Upper left corner of candlestick plot
                xanchor: 'left',
                yanchor: 'top',
                bgcolor: 'rgba(0,0,0,0.8)',
    ...
    // Add window resize listener to update chart dimensions
    if (!window.chartResizeListenerAdded) {
        window.addEventListener('resize', function() {
            const chart = document.getElementById('mainChart');
            if (chart && chart.layout) {
                const newWidth = Math.max(800, window.innerWidth - 100);
    ...
    try {
        const chartDiv = document.getElementById('mainChart');
        let preservedRanges = null;
        
        // Preserve current zoom/scaling if chart already exists
        if (chartDiv && chartDiv.layout && chartDiv.data && chartDiv.data.length > 0) {
            preservedRanges = {
                xaxis: chartDiv.layout.xaxis && chartDiv.layout.xaxis.range ? { range: [...chartDiv.layout.xaxis.range] } : null,
                yaxis: chartDiv.layout.yaxis && chartDiv.layout.yaxis.range ? { range: [...chartDiv.layout.yaxis.range] } : null,
                yaxis2: chartDiv.layout.yaxis2 && chartDiv.layout.yaxis2.range ? { range: [...chartDiv.layout.yaxis2.range] } : null,
                yaxis3: chartDiv.layout.yaxis3 && chartDiv.layout.yaxis3.range ? { range: [...chartDiv.layout.yaxis3.range] } : null,
                yaxis4: chartDiv.layout.yaxis4 && chartDiv.layout.yaxis4.range ? { range: [...chartDiv.layout.yaxis4.range] } : null
            };
            console.log("Preserved zoom ranges:", preservedRanges);
        }
        
        // Use optimized chart renderer for better performance with large datasets
        const dataSize = chartData.dates.length;
        console.log(`📊 Chart rendering with ${dataSize.toLocaleString()} data points`);
        
        if (dataSize > 10000) {
            console.log("⚡ Using WebGL-optimized rendering for large dataset");
            
            // Enhanced WebGL configuration for large datasets
            const webglConfig = {
                ...config,
                plotGLPixelRatio: window.devicePixelRatio || 1,
                toImageButtonOptions: {
                    format: 'png',
                    filename: 'divergence_chart',
                    height: 800,
                    width: 1200,
                    scale: 1
                }
            ...
            // Optimize traces for WebGL rendering with intelligent type selection
            const optimizedTraces = traces.map(trace => {
                // Use WebGL for scatter plots in large datasets
                if (trace.type === 'scatter' && (trace.mode === 'lines' || trace.mode === 'markers')) {
                    const optimizedTrace = {
                        ...trace,
                        type: dataSize > 15000 ? 'scattergl' : 'scatter',
                        // Progressive hover optimization
                        hoverinfo: dataSize > 50000 ? 'skip' : 
                                   dataSize > 30000 ? 'x+y' : trace.hoverinfo
                    };
                    
                    // Reduce marker density for extremely large datasets
                    if (dataSize > 100000 && trace.mode === 'markers' && trace.marker) {
                        optimizedTrace.marker = {
                            ...trace.marker,
                            size: Math.max(2, (trace.marker.size || 6) * 0.6), // Smaller markers
                            opacity: Math.max(0.3, (trace.marker.opacity || 1) * 0.7) // More transparent
                        };
                    }
                ...
                // Keep candlestick as regular type (WebGL candlestick not well supported)
                return trace;
            });
            ...
            const optimizedLayout = {
                ...layout,
                // Disable expensive layout features for better performance
                hovermode: dataSize > 30000 ? false : layout.hovermode,
                // Reduce animation for large datasets
                transition: { duration: dataSize > 20000 ? 0 : 500 }
            };
            
            // Render with WebGL optimizations
            Plotly.react('mainChart', optimizedTraces, optimizedLayout, webglConfig);
        } else {
            // Standard rendering for smaller datasets
            Plotly.react('mainChart', traces, layout, config);
        }
        
        // Restore preserved ranges if they existed
        if (preservedRanges) {
            const rangeUpdate = {};
            if (preservedRanges.xaxis && preservedRanges.xaxis.range) {
                rangeUpdate['xaxis.range'] = preservedRanges.xaxis.range;
            }
            if (preservedRanges.yaxis && preservedRanges.yaxis.range) {
                rangeUpdate['yaxis.range'] = preservedRanges.yaxis.range;
            }
            if (preservedRanges.yaxis2 && preservedRanges.yaxis2.range) {
                rangeUpdate['yaxis2.range'] = preservedRanges.yaxis2.range;
            }
            if (preservedRanges.yaxis3 && preservedRanges.yaxis3.range) {
                rangeUpdate['yaxis3.range'] = preservedRanges.yaxis3.range;
            }
            if (preservedRanges.yaxis4 && preservedRanges.yaxis4.range) {
                rangeUpdate['yaxis4.range'] = preservedRanges.yaxis4.range;
            }
            
            // Apply preserved ranges if any exist
            if (Object.keys(rangeUpdate).length > 0) {
                setTimeout(() => {
                    console.log("Restoring zoom ranges:", rangeUpdate);
                    Plotly.relayout('mainChart', rangeUpdate).catch(err => {
                        console.warn("Failed to restore zoom:", err);
                    });
                }, 150); // Small delay to ensure chart is rendered
            }
        }
...
function updateStats(results) {
    ...
    html += '<tr style="border-bottom: 2px solid rgba(255,255,255,0.2);"><th style="text-align: left; padding: 8px; font-size: 13px;">Variant</th><th style="text-align: center; padding: 8px; font-size: 13px;">Classic</th><th style="text-align: center; padding: 8px; font-size: 13px;">Hidden</th><th style="text-align: center; padding: 8px; font-size: 13px;">Total</th><th style="text-align: center; padding: 8px; font-size: 13px;">Capital</th><th style="text-align: center; padding: 8px; font-size: 13px;">Return %</th></tr>';

    variants.forEach(v => {
        const res = results[v.id] || { classic: [], hidden: [], total: 0 };
        
        // Calculate VISIBLE counts (respecting toggle filters) - this is what's actually displayed
        const visibleClassic = res.classic.filter(div => {
            const isBullish = div.divType === 'classic' ? true : div.macd > 0;
            return v.showClassic && ((isBullish && v.showBullish) || (!isBullish && v.showBearish));
        }).length;
        
        const visibleHidden = res.hidden.filter(div => {
            const isBullish = div.divType === 'hidden' ? (div.macd > 0) : true;
            return v.showHidden && ((isBullish && v.showBullish) || (!isBullish && v.showBearish));
        }).length;
        
        const visibleTotal = visibleClassic + visibleHidden;
        
        // Get trading result for this variant
        const tradingRes = window.allTradingResults ? window.allTradingResults[v.id] : null;
        const capitalDisplay = tradingRes ? 
            `<span style="color: ${tradingRes.totalReturn >= 0 ? '#00FF00' : '#FF0000'};">$${Math.round(tradingRes.finalCapital).toLocaleString()}</span>` : 
            '<span style="color: #888;">-</span>';
        const returnDisplay = tradingRes ? 
            `<span style="color: ${tradingRes.totalReturn >= 0 ? '#00FF00' : '#FF0000'};">${tradingRes.totalReturn >= 0 ? '+' : ''}${tradingRes.totalReturn.toFixed(1)}%</span>` : 
            '<span style="color: #888;">-</span>';
        ...
        html += `<tr style="border-bottom: 1px solid rgba(255,255,255,0.1);">
            ...
            <td style="text-align: center; padding: 8px; font-size: 13px;">${visibleClassic}</td>
            <td style="text-align: center; padding: 8px; font-size: 13px;">${visibleHidden}</td>
            <td style="text-align: center; padding: 8px; font-weight: bold; font-size: 13px;">${visibleTotal}</td>
            <td style="text-align: center; padding: 8px; font-size: 13px;">${capitalDisplay}</td>
            <td style="text-align: center; padding: 8px; font-size: 13px;">${returnDisplay}</td>
        </tr>`;
    });
    ...
    if (window.divergenceDetails && window.divergenceDetails.length > 0) {
        ...
        html += '<th style="text-align: right; padding: 6px;">Capital</th>';
        ...
        window.divergenceDetails.forEach(div => {
            ...
            html += `<tr style="border-bottom: 1px solid rgba(255,255,255,0.05); cursor: pointer; transition: background-color 0.2s;" 
                ...
                <td style="text-align: right; padding: 6px;">${div.capital ? `$${Math.round(div.capital).toLocaleString()}` : '-'}</td>
...
function refreshStyles() {
    if (variants.length === 0) {
        showMessage('messageContainer', 'No variants to refresh!', 'error');
        return;
    }
    
    if (!window.currentResults || !sessionId) {
        showMessage('messageContainer', 'No chart data to refresh. Run analysis first!', 'error');
        return;
    }
    
    // Get current styling parameter values from the form
    const textSize = parseFloat(document.getElementById('textSize').value) || 10;
    const lineWidth = parseFloat(document.getElementById('lineWidth').value) || 1.5;
    const numberPosition = parseFloat(document.getElementById('numberPosition').value) || 4;
    const namePosition = parseFloat(document.getElementById('namePosition').value) || 6;
    ...
    // Update all existing variants with new styling parameters
    variants.forEach(variant => {
        variant.textSize = textSize;
        variant.lineWidth = lineWidth;
        variant.numberPosition = numberPosition;
        variant.namePosition = namePosition;
        variant.successPosition = successPosition;
    });
    
    // Update the variants list display to show new values
    updateVariantsList();
    
    // Re-render the plot with new styling (preserves zoom/scaling)
    if (window.currentResults && window.currentChartData) {
        plotChart(window.currentChartData, window.currentResults);
    }
    
    showMessage('messageContainer', 'Styles refreshed for all variants!', 'success');
    console.log("Refreshed styles for", variants.length, "variants");
}
