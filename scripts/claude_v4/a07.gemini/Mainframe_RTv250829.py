...
def plot_markers(df, analysis_results, variant_name="V1"):
    legend_added = {}
    
    for i in range(2, len(df)):
        # CBullDivg - Classic Bullish Divergence
        if 'CBullDivg' in analysis_results:
            if "CBullD_gen" in df.columns and i < len(df) and df["CBullD_gen"].iloc[i] == 1:
                # All three subplots - same style, color, size
                fplt.plot(pd.to_datetime(df["CBullD_Lower_Low_date_gen"][i]), df["CBullD_Lower_Low_gen"][i], style="+", ax=ax1, color="red", width=2, legend=f"{variant_name} Bullish" if f"{variant_name} Bullish" not in legend_added else None)
                fplt.plot(pd.to_datetime(df["CBullD_Higher_Low_date_gen"][i]), df["CBullD_Higher_Low_gen"][i], style="+", ax=ax1, color="blue", width=2)
                fplt.plot(pd.to_datetime(df["CBullD_Lower_Low_date_gen"][i]), df["CBullD_Lower_Low_RSI_gen"][i], style="+", ax=ax2, color="red", width=2)
                fplt.plot(pd.to_datetime(df["CBullD_Higher_Low_date_gen"][i]), df["CBullD_Higher_Low_RSI_gen"][i], style="+", ax=ax2, color="blue", width=2)
                fplt.plot(pd.to_datetime(df["CBullD_Lower_Low_date_gen"][i]), df["CBullD_Lower_Low_MACD_gen"][i], style="+", ax=ax3, color="red", width=2)
                fplt.plot(pd.to_datetime(df["CBullD_Higher_Low_date_gen"][i]), df["CBullD_Higher_Low_MACD_gen"][i], style="+", ax=ax3, color="blue", width=2)
                legend_added[f"{variant_name} Bullish"] = True
            
            if "CBullD_neg_MACD" in df.columns and i < len(df) and df["CBullD_neg_MACD"].iloc[i] == 1:
                # All three subplots - same style, color, size
                fplt.plot(pd.to_datetime(df["CBullD_Lower_Low_date_neg_MACD"][i]), df["CBullD_Lower_Low_neg_MACD"][i], style="x", ax=ax1, color="red", width=2)
                fplt.plot(pd.to_datetime(df["CBullD_Higher_Low_date_neg_MACD"][i]), df["CBullD_Higher_Low_neg_MACD"][i], style="x", ax=ax1, color="blue", width=2)
        ...
        # CBullDivg_x2 - Extended Bullish (Triangle markers)
        if 'CBullDivg_x2' in analysis_results:
            if "CBullD_x2_gen" in df.columns and i < len(df) and df["CBullD_x2_gen"].iloc[i] == 1:
                fplt.plot(pd.to_datetime(df["CBullD_x2_Lower_Low_date_gen"][i]), df["CBullD_x2_Lower_Low_gen"][i], style="^", ax=ax1, color="red", width=1, legend="Extended Bullish" if "Extended Bullish" not in legend_added else None)
                fplt.plot(pd.to_datetime(df["CBullD_x2_Higher_Low_date_gen"][i]), df["CBullD_x2_Higher_Low_gen"][i], style="^", ax=ax1, color="blue", width=1)
                ...
                legend_added["Extended Bullish"] = True
        
        # HBearDivg - Hidden Bearish (Square markers)
        if 'HBearDivg' in analysis_results:
            if "HBearD_gen" in df.columns and i < len(df) and df["HBearD_gen"].iloc[i] == 1:
                fplt.plot(pd.to_datetime(df["HBearD_Higher_High_date_gen"][i]), df["HBearD_Higher_High_gen"][i], style="s", ax=ax1, color="red", width=1, legend="Hidden Bearish" if "Hidden Bearish" not in legend_added else None)
                fplt.plot(pd.to_datetime(df["HBearD_Lower_High_date_gen"][i]), df["HBearD_Lower_High_gen"][i], style="s", ax=ax1, color="blue", width=1)
                ...
                legend_added["Hidden Bearish"] = True
        
        # HBullDivg - Hidden Bullish (Circle markers)
        if 'HBullDivg' in analysis_results:
            if "HBullD_gen" in df.columns and i < len(df) and df["HBullD_gen"].iloc[i] == 1:
                fplt.plot(pd.to_datetime(df["HBullD_Lower_Low_date_gen"][i]), df["HBullD_Lower_Low_gen"][i], style="o", ax=ax1, color="red", width=1, legend="Hidden Bullish 2" if "Hidden Bullish 2" not in legend_added else None)
                fplt.plot(pd.to_datetime(df["HBullD_Higher_Low_date_gen"][i]), df["HBullD_Higher_Low_gen"][i], style="o", ax=ax1, color="blue", width=1)
                ...
                legend_added["Hidden Bullish 2"] = True
...
if analysis_type != 'f':
    ...
    fplt.bull_color = '#44ff44'  # Green rising candles
    fplt.bear_color = '#ff4444'  # Red falling candles
    ...
    # Create candlesticks (finplot handles colors via global settings)
    ...
    plot_markers(df, analysis_results, "V1")
    ...
    # CONSOLIDATE ALL RESULTS IN ONE MASTER XLSX
    os.makedirs('results', exist_ok=True)
    master_xlsx = os.path.join('results', f"MASTER_RESULTS_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}.xlsx")
    if openpyxl:
        all_data = []
        # Add main results
        ...
        # Add variant2 results to master if exists
        if variant2:
            variant2_counts = {}  # Will be populated below
        
        with pd.ExcelWriter(master_xlsx, engine='openpyxl') as writer:
            df_master = pd.DataFrame(all_data)
            df_master.to_excel(writer, sheet_name='Summary', index=False)
            print(f"\nMASTER RESULTS exported to {master_xlsx}")
    
    # TEXT BOX WITH MARKER COUNTS - MIDDLE LEFT (avoid legend overlap completely)
    count_text = "\n".join([f"{name}: {data['total']}" for name, data in counts.items()])
    mid_price = (df['high'].max() + df['low'].min()) / 2
    fplt.add_text((df.index[5], mid_price), count_text, ax=ax1, color='white')
    ...
    if variant2:
        ...
        # Plot variant2 markers with V2 legend group
        plot_markers(df_var2, variant2_results, "V2")
        ...
        fplt.add_text((df.index[10], df['high'].max() * 0.85), variant2_text, ax=ax1, color='yellow')
        
        # Add variant2 to master XLSX
        if openpyxl:
            variant2_data = []
            for analysis in variant2_counts:
                variant2_data.append({
                    'Run_Type': 'Variant2',
            ...
            # Update master file with variant2 data
            with pd.ExcelWriter(master_xlsx, engine='openpyxl', mode='a', if_sheet_exists='overlay') as writer:
                df_variant2 = pd.DataFrame(variant2_data)
                existing_df = pd.read_excel(master_xlsx, sheet_name='Summary')
                combined_df = pd.concat([existing_df, df_variant2], ignore_index=True)
                combined_df.to_excel(writer, sheet_name='Summary', index=False)
    
    # Yellow/Blue comparison circles - compare variant2 vs main variant  
    if variant2:
        print("\nAdding comparison circles...")
        
        # Get main variant positions (all analysis types from main run)
        main_positions = set()
        for i in range(len(df)):
            if "CBullD_gen" in df.columns and df["CBullD_gen"].iloc[i] == 1:
                main_positions.add(i)
            if "CBullD_neg_MACD" in df.columns and df["CBullD_neg_MACD"].iloc[i] == 1:
                main_positions.add(i)
        ...
        # Get variant2 positions
        variant2_positions = set()
        for i in range(len(df_var2)):
            if "CBullD_gen" in df_var2.columns and df_var2["CBullD_gen"].iloc[i] == 1:
                variant2_positions.add(i)
            if "CBullD_neg_MACD" in df_var2.columns and df_var2["CBullD_neg_MACD"].iloc[i] == 1:
                variant2_positions.add(i)
            if "CBullD_x2_gen" in df_var2.columns and df_var2["CBullD_x2_gen"].iloc[i] == 1:
                variant2_positions.add(i)
            if "HBearD_gen" in df_var2.columns and df_var2["HBearD_gen"].iloc[i] == 1:
                variant2_positions.add(i)
            if "HBullD_gen" in df_var2.columns and df_var2["HBullD_gen"].iloc[i] == 1:
                variant2_positions.add(i)
        
        # YELLOW circles for additional in variant2, BLUE circles for missing in variant2
        additional = variant2_positions - main_positions
        missing = main_positions - variant2_positions
        
        print(f"Additional markers in variant2: {len(additional)}")
        print(f"Missing markers in variant2: {len(missing)}")
        
        # TRANSPARENT YELLOW/BLUE CIRCLES around markers (outline only)
        import matplotlib.pyplot as plt
        for pos in additional:
            if pos < len(df):
                # Use matplotlib circle with no fill
                circle = plt.Circle((pd.to_datetime(df["date"].iloc[pos]).timestamp(), df["high"].iloc[pos] * 1.03), 
                                  radius=df["high"].iloc[pos] * 0.01, color="yellow", fill=False, linewidth=2)
                fplt.plot(pd.to_datetime(df["date"].iloc[pos]), df["high"].iloc[pos] * 1.03, 
                         style="o", ax=ax1, color="yellow", width=1, legend="○ Additional" if pos == list(additional)[0] else None)
        
        for pos in missing:
            if pos < len(df):
                fplt.plot(pd.to_datetime(df["date"].iloc[pos]), df["low"].iloc[pos] * 0.97, 
                         style="o", ax=ax1, color="lightblue", width=1, legend="○ Missing" if pos == list(missing)[0] else None)
    
    # WORKING RESET FUNCTIONALITY - Press R key or double-click plot
    def reset_chart():
        fplt.set_y_range(df['low'].min() * 0.98, df['high'].max() * 1.02, ax=ax1)
        fplt.set_y_range(0, 100, ax=ax2) 
        fplt.set_y_range(df['macd_histogram'].min() * 1.1, df['macd_histogram'].max() * 1.1, ax=ax3)
    
    # Add reset instructions in upper right
    fplt.add_text((df.index[-80], df['high'].max() * 0.97), "R = Reset View", ax=ax1, color='lime')
    
    # Bind R key to reset function
    import pyqtgraph as pg
    def keyPressEvent(ev):
        if ev.text() == 'r' or ev.text() == 'R':
            reset_chart()
    
    # Enable keyboard events
    try:
        fplt.win.keyPressEvent = keyPressEvent
    except:
        pass
    ...
    if os.path.exists(doe_file):
        ...
        fplt.add_text((df_best.index[10], df_best['high'].max() * 0.95), count_text, ax=ax1, color='white')
        
        fplt.add_text((df_best.index[-50], df_best['high'].max() * 0.95), "[RESET]", ax=ax1, color='red')
