...
# Load data supporting both CSV and Parquet
...
if analysis_type != 'f':
    ...
    if variant2:
        ...
        for date, price in additional:
            # Use larger circles and proper positioning
            date_num = mdates.date2num(date) if hasattr(date, 'year') else mdates.date2num(pd.to_datetime(date))
            # Fixed radius calculation for better visibility - use fixed size based on data range
            price_range = df['high'].max() - df['low'].min()
            circle_radius = price_range * 0.01  # 1% of price range for consistent visibility
            circle = Circle((date_num, price), radius=circle_radius, 
                          facecolor='none', edgecolor='yellow', linewidth=4, alpha=1.0)
            ax1.add_patch(circle)
            print(f"Added YELLOW circle at {date}, price {price}, radius {circle_radius:.2f}")
        ...
        for date, price in missing:
            ...
            circle_radius = price_range * 0.01  # 1% of price range for consistent visibility
            circle = Circle((date_num, price), radius=circle_radius,
                          facecolor='none', edgecolor='blue', linewidth=4, alpha=1.0)  
            ax1.add_patch(circle)
            print(f"Added BLUE circle at {date}, price {price}, radius {circle_radius:.2f}")
    ...
    # LEGEND REORGANIZED FOR LOGICAL GROUPING (LIKE XLSX) - TOGGABLE AND FULLY VISIBLE
    # Get all legend handles and labels, then sort them logically
    handles, labels = ax1.get_legend_handles_labels()
    
    # Sort labels by variant first, then by analysis type
    sorted_legend_items = sorted(zip(handles, labels), key=lambda x: (
        x[1].split()[0],  # Variant name (V1, V2)
        x[1].split()[1],  # Analysis type (CBullDivg, HBearDivg, etc.)
        x[1].split()[-1] if len(x[1].split()) > 2 else ''  # Classic/Hidden
    ))
    
    sorted_handles, sorted_labels = zip(*sorted_legend_items) if sorted_legend_items else ([], [])
    
    legend = ax1.legend(sorted_handles, sorted_labels, loc='upper right', 
                       facecolor='black', edgecolor='white', labelcolor='white', 
                       fontsize=8, ncol=2, bbox_to_anchor=(0.98, 0.98), 
                       framealpha=0.9, columnspacing=1.0)
    legend.set_picker(True)
    
    # Make legend entries toggable - FIXED VERSION
    def on_legend_pick(event):
        legend_item = event.artist
        if hasattr(legend_item, 'get_label'):
            ...
            for ax in [ax1, ax2, ax3]:
                for artist in ax.get_children():
                    if (hasattr(artist, 'get_label') and 
                        hasattr(artist, 'set_visible') and 
                        artist.get_label() == label and 
                        artist != legend_item):
                        artist.set_visible(not artist.get_visible())
            
            # Update legend item appearance
            current_alpha = legend_item.get_alpha()
            legend_item.set_alpha(0.3 if current_alpha == 1.0 else 1.0)
            fig.canvas.draw()
    ...
    # Enable interactive navigation with zoom/pan
