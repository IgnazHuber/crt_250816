...
# Korrigierte Vollversion – keine Features entfernt, nur Fixes/Robustheit
...
def add_markers_to_plotly(fig, df, analysis_results, variant_name):
    ...
    def add_trace(dates, y_price, y_rsi, y_macd, label_for_showlegend, style, direction, div_type, size=12, opacity=1.0):
        legend_group = f"{variant_name}/{div_type}/{direction}"
        legend_title = f"{variant_name} - {div_type} - {direction}"
        # showlegend nur einmal pro Gruppe (Classic oder x2)
        show_legend = label_for_showlegend in ('Classic', 'x2')
    ...
    for i in range(len(df)):
        ...
        if analysis_results.get('CBullDivg', False) and 'CBullD_gen' in df.columns and df.at[i, 'CBullD_gen'] == 1:
            ...
            if pd.notna(low1) and pd.notna(low2):
                add_trace([d1, d2], [low1 * 0.99, low2 * 0.99], [rsi1, rsi2], [macd1, macd2],
                          'Classic', marker_styles['CBullDivg'], 'Bullish', 'CBullDivg', size=12, opacity=1.0)
        ...
        if analysis_results.get('CBullDivg', False) and 'CBullD_neg_MACD' in df.columns and df.at[i, 'CBullD_neg_MACD'] == 1:
            ...
            low2, _, rsi2, macd2 = _safe_get_vals(df_idx, d2)
            if pd.notna(low1) and pd.notna(low2):
                add_trace([d1, d2], [low1 * 0.98, low2 * 0.98], [rsi1, rsi2], [macd1, macd2],
                          'Hidden', marker_styles['CBullDivg'], 'Bullish', 'CBullDivg', size=10, opacity=0.7)
        ...
        if analysis_results.get('CBullDivg_x2', False) and 'CBullD_x2_gen' in df.columns and df.at[i, 'CBullD_x2_gen'] == 1:
            ...
            low2, _, rsi2, macd2 = _safe_get_vals(df_idx, d2)
            if pd.notna(low1) and pd.notna(low2):
                add_trace([d1, d2], [low1 * 0.97, low2 * 0.97], [rsi1, rsi2], [macd1, macd2],
                          'x2', marker_styles['CBullDivg_x2'], 'Bullish', 'CBullDivg_x2', size=12, opacity=1.0)
        ...
        if analysis_results.get('HBearDivg', False) and 'HBearD_gen' in df.columns and df.at[i, 'HBearD_gen'] == 1:
            ...
            if pd.notna(high1) and pd.notna(high2):
                add_trace([d1, d2], [high1 * 1.01, high2 * 1.01], [rsi1, rsi2], [macd1, macd2],
                          'Classic', marker_styles['HBearDivg'], 'Bearish', 'HBearDivg', size=12, opacity=1.0)
        ...
        if analysis_results.get('HBullDivg', False) and 'HBullD_gen' in df.columns and df.at[i, 'HBullD_gen'] == 1:
            ...
            low2, _, rsi2, macd2 = _safe_get_vals(df_idx, d2)
            if pd.notna(low1) and pd.notna(low2):
                add_trace([d1, d2], [low1 * 0.96, low2 * 0.96], [rsi1, rsi2], [macd1, macd2],
                          'Classic', marker_styles['HBullDivg'], 'Bullish', 'HBullDivg', size=12, opacity=1.0)
        ...
        if analysis_results.get('HBullDivg', False) and 'HBullD_neg_MACD' in df.columns and df.at[i, 'HBullD_neg_MACD'] == 1:
            ...
            low2, _, rsi2, macd2 = _safe_get_vals(df_idx, d2)
            if pd.notna(low1) and pd.notna(low2):
                add_trace([d1, d2], [low1 * 0.95, low2 * 0.95], [rsi1, rsi2], [macd1, macd2],
                          'Hidden', marker_styles['HBullDivg'], 'Bullish', 'HBullDivg', size=10, opacity=0.7)
...
def plot_with_plotly(df_main, analysis_results, counts_main, df_var2=None, variant2_results=None):
    ...
    for col, col_color in ema_defs.items():
    ...
    if df_var2 is not None and variant2_results is not None:
        ...
        # Helper-Funktion für Plot der Outlines in allen Subplots
        def _add_outline(dates_prices, name, color):
            if not dates_prices:
                return
            dates, prices = zip(*dates_prices)
            # RSI/MACD-Werte für dieselben Zeitpunkte (aus df_main, konsistent)
            ...
            fig.add_trace(go.Scatter(x=list(dates), y=list(prices), mode='markers', name=name,
                                     marker=mk, legendgroup='Comparison', legendgrouptitle_text='Comparison'),
                          row=1, col=1)
            fig.add_trace(go.Scatter(x=list(dates), y=rsi_vals, mode='markers', name=name,
                                     marker=mk, legendgroup='Comparison', showlegend=False),
                          row=2, col=1)
            fig.add_trace(go.Scatter(x=list(dates), y=macd_vals_local, mode='markers', name=name,
                                     marker=mk, legendgroup='Comparison', showlegend=False),
                          row=3, col=1)
