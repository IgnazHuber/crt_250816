...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function handleDrop(e) {
        ...
        if (files.length > 0) {
            ...
            if (['csv', 'parquet', 'pq'].includes(fileExt)) {
...
function plotChart(chartData, results) {
    ...
    variants.forEach((variant, variantIndex) => {
        ...
        allDivergences.forEach(div => {
            ...
            // Define marker symbols: CBullDivg=cross, CBullDivg_x2=triangle, HBullDivg=square, HBearDivg=diamond
            // Define colors: bullish=green, bearish=red
            const markerSymbol = (() => {
                if (div.analysis === 'CBullDivg') return 'x';  // cross
                if (div.analysis === 'CBullDivg_x2') return 'triangle-up';  // triangle
                if (div.analysis === 'HBullDivg') return 'square';  // square
                if (div.analysis === 'HBearDivg') return 'diamond';  // rhombic
                return 'circle';  // fallback
            })();
            
            const markerColor = isBullish ? '#00FF00' : '#FF0000';  // green for bullish, red for bearish
            
            // Add main marker with new styling
            traces.push({
                x: [div.date],
                y: [div.low],
                mode: 'markers',
                type: 'scatter',
                name: `${variant.name} ${div.analysis} ${div.type}`,
                marker: {
                    size: 16,
                    color: markerColor,
                    symbol: markerSymbol,
                ...
                xaxis: 'x',
                yaxis: 'y',
                legendgroup: `${variant.name}_${div.analysis}`,
    ...
    // Run bullish trading strategy simulation for each variant
    ...
    // Add capital tracking plots for each variant
    ...
    if (tradingResult && tradingResult.tradeHistory) {
