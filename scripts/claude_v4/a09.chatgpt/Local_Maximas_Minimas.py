...
def Local_Max_Min(ohlc: pd.DataFrame) -> pd.Series:
    ...
    for i in range(window_1, n - window_1):
        if all(CH[i] > CH[j] for j in range(i - window_1, i)) and all(
            CH[i] >= CH[j] for j in range(i + 1, i + window_1 + 1)
        ):  # Searching for maximas on a symmetric window
    ...
    for i in range(window_2, n - window_2):
        if all(CH[i] > CH[j] for j in range(i - window_2, i)) and all(
            CH[i] >= CH[j] for j in range(i + 1, i + window_2 + 1)
        ):  # Searching for maximas on a symmetric window
    ...
    for i in range(window_1, n - window_1):
        if all(CL[i] < CL[j] for j in range(i - window_1, i)) and all(
            CL[i] <= CL[j] for j in range(i + 1, i + window_1 + 1)
        ):  # Searching for minimas on a symmetric window
    ...
    for i in range(window_2, n - window_2):
        if all(CL[i] < CL[j] for j in range(i - window_2, i)) and all(
            CL[i] <= CL[j] for j in range(i + 1, i + window_2 + 1)
        ):  # Searching for minimas on a symmetric window
    ...
    for i in range(window_1, n - window_1):
        if all(macd[i] > macd[j] for j in range(i - window_1, i)) and all(
            macd[i] >= macd[j] for j in range(i + 1, i + window_1 + 1)
        ):  # Searching for maximas on a symmetric window
    ...
    for i in range(window_2, n - window_2):
        if all(macd[i] > macd[j] for j in range(i - window_2, i)) and all(
            macd[i] >= macd[j] for j in range(i + 1, i + window_2 + 1)
        ):  # Searching for maximas on a symmetric window
    ...
    non_zero_high_indices = [
        i for i, v in enumerate(Level_1_High_window_1_CS) if v != 0
    ]
    for idx, i in enumerate(non_zero_high_indices):
        ...
        if (
            idx > 0
            and current_val <= Level_1_High_window_1_CS[non_zero_high_indices[idx - 1]]
        ):
            is_peak = False
        if (
            idx < len(non_zero_high_indices) - 1
            and current_val <= Level_1_High_window_1_CS[non_zero_high_indices[idx + 1]]
        ):
            is_peak = False
    ...
    for idx, i in enumerate(non_zero_low_indices):
        ...
        if (
            idx > 0
            and current_val >= Level_1_Low_window_1_CS[non_zero_low_indices[idx - 1]]
        ):
            is_trough = False
        if (
            idx < len(non_zero_low_indices) - 1
            and current_val >= Level_1_Low_window_1_CS[non_zero_low_indices[idx + 1]]
        ):
            is_trough = False
