...
def prompt_params() -> BacktestParams:
    ...
    return BacktestParams(risk_pct=risk, stop_pct=sl, tp_pct=tp, fee_pct=fee, slippage_pct=slip)
...
def backtest(df_ohlc: pd.DataFrame, markers: pd.DataFrame, params: BacktestParams) -> Dict[str, Any]:
    ...
    trades = []
    open_trades = []  # list of dicts with exit_i, pnl, exit_date
    ...
    for _, row in mk.iterrows():
        ...
        # single-position mode: skip if any trade still open
        ...
        open_trades.append({'exit_i': exit_i, 'pnl': pnl, 'exit_date': exit_date})
    ...
    if equity_points:
        eq_df = pd.DataFrame(equity_points, columns=['Date', 'Equity']).sort_values('Date')
    else:
    ...
    summary = pd.DataFrame([
        {
            ...
            'Final_Equity': params.starting_capital + total_pnl,
            'Total_PnL': total_pnl,
    ...
    return {
        'trades': trades_df,
        'equity': eq_df[['Date', 'Equity']] if not eq_df.empty else eq_df,
        'summary': summary,
        'by_type': by_type,
    }
...
def export_backtest_xlsx(results: Dict[str, Any], out_path: str):
    ...
    with pd.ExcelWriter(out_path, engine='openpyxl') as writer:
        ...
        if not equity.empty:
            ...
            if len(equity) >= 2:
                data_ref = Reference(ws, min_col=2, min_row=1, max_col=2, max_row=len(equity) + 1)
                chart = LineChart()
