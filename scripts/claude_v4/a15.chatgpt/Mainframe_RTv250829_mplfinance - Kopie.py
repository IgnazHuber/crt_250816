...
def get_analysis_type():
    ...
    try:
        ...
        while True:
            c = input("\nEnter your choice (a-g): ").lower().strip()
            if c in list("abcdefg"):
                return c
...
if __name__ == "__main__":
    ...
    if atype not in ('f', 'g'):
        ...
        try:
            ...
            print(f"\nBacktesting with: risk={bt_params.risk_pct}%, stop={bt_params.stop_pct}%, tp={bt_params.tp_pct}%, fee/side={bt_params.fee_pct}%, slippage={bt_params.slippage_pct}%")
            ...
            # XLSX export
            out_xlsx = os.path.join('results', f"backtest_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx")
