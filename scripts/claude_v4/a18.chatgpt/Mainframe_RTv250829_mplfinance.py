...
$form.Controls.AddRange(@($lbl1,$dp1,$lbl2,$dp2,$ok,$cancel))
...
$sel = $null
$ok.Add_Click({ $sel = "OK"; $form.Close() })
$cancel.Add_Click({ $sel = "CANCEL"; $form.Close() })

$form.Topmost = $true
$form.Add_Shown({ $form.Activate() })
[void]$form.ShowDialog()

if ($sel -eq "OK") { Write-Output ("{0}|{1}" -f $dp1.Value.ToString("o"), $dp2.Value.ToString("o")) }
'''
    ...
    # Console fallback
    try:
        print("\nEnter backtest date range (YYYY-MM-DD). Press Enter for defaults.")
        s = input(f"Start date [{min_dt.date()}]: ").strip()
        e = input(f"End date   [{max_dt.date()}]: ").strip()
        s_dt = pd.to_datetime(s) if s else min_dt
        e_dt = pd.to_datetime(e) if e else max_dt
        return pd.to_datetime(s_dt, utc=True), pd.to_datetime(e_dt, utc=True)
...
def _doe_plot_by_type(df_counts, single_pages=False):
    """
...
def run_doe_analysis(df, doe_params_file="doe_parameters_example.csv",
                     make_total=False, make_single=False, parallel=True):
    """
    ...
    if not parallel or len(indexed_params) <= 1:
        ...
        # Parallel execution of independent DOE iterations
        ...
        print(f"Running DOE in parallel with {max_workers} workers...")
        futures = []
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            for idx, candle_tol, macd_tol in indexed_params:
                print(f"Queueing DOE iteration {idx + 1}: candle_tol={candle_tol}, macd_tol={macd_tol}")
                futures.append(executor.submit(_doe_worker, idx, candle_tol, macd_tol, df, window))

            # Collect results, then post-process in original order
    ...
    if all_counts_rows:
        ...
        if make_total:
            _doe_plot_total(df_counts)          # optional Gesamt-Heatmap
        _doe_plot_by_type(df_counts, single_pages=make_single)   # Facet (immer)
...
if __name__ == "__main__":
    ...
    if atype not in ('f', 'g', 'h'):
        ...
        run_doe_analysis(df, make_total=False, make_single=False)
    else:
        # Backtesting path
        ...
        try:
            ...
            out_xlsx = os.path.join('results', f"{base_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx")
