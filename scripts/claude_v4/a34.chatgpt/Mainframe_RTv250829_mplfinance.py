...
def get_analysis_parameters():
    ...
    try:
        ...
        s3 = input("Enter third Candle tolerance % (leave empty for none): ").strip()
        s4 = input("Enter third MACD tolerance % (leave empty for none): ").strip()
...
def get_analysis_type():
    ...
    try:
        ...
        while True:
            c = input("\nEnter your choice (a-i): ").lower().strip()
            if c in list("abcdefghi"):
                return c
            print("Invalid choice. Please enter a, b, c, d, e, f, g, h, or i.")
    except (EOFError, KeyboardInterrupt):
        return "e"
...
def plot_with_plotly(df_main, analysis_results, counts_main, df_var2=None, variant2_results=None, counts_var2=None, asset_label=None,
    ...
    # Additional asset info: Volume and ATR%
    ...
    fig = make_subplots(
        ...
        subplot_titles=('Price', 'RSI', 'MACD Histogram', 'Volume', 'ATR %'),
        row_heights=[0.35, 0.2, 0.2, 0.15, 0.1]
    )
    ...
    if 'volume' in df_main.columns:
        ...
        try:
            ...
            vol_colors = '#888888'
        fig.add_trace(
            go.Bar(x=df_main.index, y=df_main['volume'], name='Volume', marker_color=vol_colors),
            row=4, col=1
        )
    ...
    if 'ATR_Pct' in df_main.columns:
        fig.add_trace(
            go.Scatter(x=df_main.index, y=df_main['ATR_Pct'], mode='lines', name='ATR %', line=dict(width=1.5), opacity=0.9),
            row=5, col=1
        )
    ...
    main_positions = add_markers_to_plotly(
        fig, df_m_for_markers, analysis_results, 'V1 (Main)',
        extra_targets=[{'row': 4, 'key': 'volume'}, {'row': 5, 'key': 'ATR_Pct'}]
    )
    ...
    if df_var2 is not None and variant2_results is not None:
        ...
        v2_positions = add_markers_to_plotly(
            fig, df_v2_for_markers, variant2_results, 'V2 (Variant)',
            extra_targets=[{'row': 4, 'key': 'volume'}, {'row': 5, 'key': 'ATR_Pct'}]
        )
    ...
    if df_var3 is not None and variant3_results is not None:
        ...
        v3_positions = add_markers_to_plotly(
            fig, df_v3_for_markers, variant3_results, 'V3 (Variant 3)',
            extra_targets=[{'row': 4, 'key': 'volume'}, {'row': 5, 'key': 'ATR_Pct'}]
        )
    ...
    try:
        ...
        hover_buttons = [
            ...
            dict(label='Extended hover', method='relayout', args=[{'hovermode': 'x unified'}]),
        ]
    ...
    if counts_var3:
        box_parts.append(_format_counts_box("V3 (Variant 3)", counts_var3))
...
if __name__ == "__main__":
    ...
    if atype not in ('f', 'g', 'h', 'i'):
        ...
        plot_with_plotly(df, analysis_results, counts, df2, res2, counts2, asset_label=asset_label, df_var3=df3, variant3_results=res3, counts_var3=counts3)
