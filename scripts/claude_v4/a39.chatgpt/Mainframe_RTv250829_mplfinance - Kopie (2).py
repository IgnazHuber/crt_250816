...
def plot_with_plotly(df_main, analysis_results, counts_main, df_var2=None, variant2_results=None, counts_var2=None, asset_label=None,
    ...
    try:
        ...
        # Compute annotation lists for info show/hide (info annotation added last)
        anns_show = None
        anns_hide = None
        try:
            _anns = list(fig.layout.annotations) if fig.layout.annotations is not None else []
            anns_show = [a.to_plotly_json() for a in _anns]
            anns_hide = [a.to_plotly_json() for a in _anns]
            if len(anns_hide) >= 1:
                anns_hide[-1]['opacity'] = 0.0
        except Exception:
            pass
        ...
        fig.update_layout(
            updatemenus=[
                ...
                dict(
                    ...
                    buttons=(
                        [
                            dict(label='Show Info', method='relayout', args=[{'annotations': anns_show}]),
                            dict(label='Hide Info', method='relayout', args=[{'annotations': anns_hide}])
                        ] if (anns_show is not None and anns_hide is not None) else []
                    ),
                    pad=dict(r=5, t=2),
                    showactive=False,
                )
