...
def plot_with_plotly(df_main, analysis_results, counts_main, df_var2=None, variant2_results=None, counts_var2=None, asset_label=None,
    ...
    downsample = len(df_main.index) > gl_threshold and downsample_factor > 1
    ...
    def downsample_data(x, y, factor=downsample_factor):
        ...
        if not downsample or len(x) <= factor:
            return x, y
        indices = np.arange(0, len(x), factor)
        return x.iloc[indices] if hasattr(x, 'iloc') else x[indices], y.iloc[indices] if hasattr(y, 'iloc') else y[indices]
    ...
    for col, _col_color in ema_defs.items():
        if col in df_main.columns:
            ...
            fig.add_trace(LineTrace(x_ds, y_ds, col, width=1, opacity=0.8), row=1, col=1)
