...
def add_markers_to_plotly(fig, df, analysis_results, variant_name, extra_targets=None, use_gl=False):
    """
    ...
    def _add_pair(dates, y_price, y_rsi, y_macd, div_type, direction, size, opacity):
        ...
        # Choose Scatter or Scattergl based on use_gl
        ScatterClass = go.Scattergl if use_gl else go.Scatter
        # Preis
        price_trace = ScatterClass(x=dates, y=y_price, mode='markers',
                        name=f"{div_type} - {direction}",
                        marker=common_marker,
                        legendgroup=gid,
                        showlegend=False)
        price_trace.hovertemplate = f"{div_type} {direction}<br>Price: %{{y:.4f}}<br>Date: %{{x}}<extra></extra>"
        price_trace.hoverinfo = 'text' if use_gl else 'all'
        fig.add_trace(price_trace, row=1, col=1)

        # RSI
        rsi_trace = ScatterClass(x=dates, y=y_rsi, mode='markers',
                        name=f"{div_type} - {direction}",
                        marker=common_marker,
                        legendgroup=gid,
                        showlegend=False)
        rsi_trace.hovertemplate = f"{div_type} {direction}<br>RSI: %{{y:.1f}}<extra></extra>"
        rsi_trace.hoverinfo = 'text' if use_gl else 'all'
        fig.add_trace(rsi_trace, row=2, col=1)

        # MACD
        macd_trace = ScatterClass(x=dates, y=y_macd, mode='markers',
                        name=f"{div_type} - {direction}",
                        marker=common_marker,
                        legendgroup=gid,
                        showlegend=False)
        macd_trace.hovertemplate = f"{div_type} {direction}<br>MACD: %{{y:.6f}}<extra></extra>"
        macd_trace.hoverinfo = 'text' if use_gl else 'all'
        fig.add_trace(macd_trace, row=3, col=1)
                ...
                if row_idx and key:
                    ...
                    for d in dates:
                        try:
                            ...
                            ys.append(np.nan)
                    trace = ScatterClass(x=dates, y=ys, mode='markers',
                                    name=f"{div_type} - {direction}",
                                    marker=common_marker,
                                    legendgroup=gid,
                                    showlegend=False)
                    trace.hovertemplate = f"{div_type} {direction}<br>{key}: %{{y:.4f}}<extra></extra>"
                    trace.hoverinfo = 'text' if use_gl else 'all'
...
def plot_with_plotly(df_main, analysis_results, counts_main, df_var2=None, variant2_results=None, counts_var2=None, asset_label=None,
    ...
    # Allow disabling optimizations if needed
    disable_optimizations = os.getenv('PLOT_DISABLE_OPTIMIZATIONS', '').lower() in ('1', 'true', 'yes')
    use_gl = len(df_main.index) > gl_threshold and not disable_optimizations

    # Downsampling for large datasets
    try:
        downsample_factor = int(os.getenv('PLOT_DOWNSAMPLE_FACTOR', '10'))
    except Exception:
        downsample_factor = 10
    downsample = len(df_main.index) > gl_threshold and downsample_factor > 1 and not disable_optimizations

    # Debug output for optimizations
    if os.getenv('PLOT_DEBUG', '').lower() in ('1', 'true', 'yes'):
        print(f"[PLOT OPTIMIZATION] Dataset size: {len(df_main.index)}")
        print(f"[PLOT OPTIMIZATION] GL threshold: {gl_threshold}")
        print(f"[PLOT OPTIMIZATION] Use GL: {use_gl}")
        print(f"[PLOT OPTIMIZATION] Downsample: {downsample}")
        print(f"[PLOT OPTIMIZATION] Downsample factor: {downsample_factor}")
        print(f"[PLOT OPTIMIZATION] Disable optimizations: {disable_optimizations}")
    ...
    def downsample_data(x, y, factor=downsample_factor):
        """Downsample data by taking every factor-th point."""
        if not downsample or len(x) <= factor or factor <= 1:
            return x, y
        try:
            ...
            if hasattr(x, 'iloc'):
                ...
                x_ds = x[indices] if hasattr(x, '__getitem__') else x
            if hasattr(y, 'iloc'):
                y_ds = y.iloc[indices]
            else:
                y_ds = y[indices] if hasattr(y, '__getitem__') else y
            return x_ds, y_ds
        except Exception:
            # Fallback to original data if downsampling fails
            return x, y
    ...
    for col, _col_color in ema_defs.items():
        if col in df_main.columns:
            x_ds, y_ds = downsample_data(df_main.index, df_main[col])
            trace = LineTrace(x_ds, y_ds, col, width=1, opacity=0.8)
            # Optimize hover for performance
            trace.hovertemplate = f"{col}: %{{y:.2f}}<extra></extra>"
            trace.hoverinfo = 'y' if use_gl else 'all'
            fig.add_trace(trace, row=1, col=1)

    # RSI
    if 'RSI' in df_main.columns:
        x_ds, y_ds = downsample_data(df_main.index, df_main['RSI'])
        trace = LineTrace(x_ds, y_ds, 'RSI', width=2, opacity=1.0)
        trace.hovertemplate = "RSI: %{y:.1f}<extra></extra>"
        trace.hoverinfo = 'y' if use_gl else 'all'
    ...
    if use_gl:
        # Use Scattergl for better performance on large datasets
        x_ds, y_ds = downsample_data(df_main.index, macd_vals)
        # Create positive and negative series separately for better performance
        pos_mask = y_ds >= 0
        neg_mask = y_ds < 0

        if pos_mask.any():
            fig.add_trace(
                go.Scattergl(x=x_ds[pos_mask], y=y_ds[pos_mask], mode='lines', name='MACD Hist. (+)',
                            line=dict(color='#00FF00', width=0), fill='tozeroy', fillcolor='rgba(0,255,0,0.3)',
                            showlegend=False),
                row=3, col=1
            )
        if neg_mask.any():
            fig.add_trace(
                go.Scattergl(x=x_ds[neg_mask], y=y_ds[neg_mask], mode='lines', name='MACD Hist. (-)',
                            line=dict(color='#FF0000', width=0), fill='tozeroy', fillcolor='rgba(255,0,0,0.3)',
                            showlegend=False),
                row=3, col=1
            )
    else:
    ...
    for idx, (title, key, _px) in enumerate(panels, start=1):
        ...
        if key == 'volume':
            ...
            if use_gl:
                # Use Scattergl for better performance on large datasets
                x_ds, y_ds = downsample_data(df_main.index, df_main['volume'])
                # Create up and down volume series separately
                up_mask = (df_main['close'] >= df_main['close'].shift(1)).fillna(True)
                up_x = df_main.index[up_mask]
                up_y = df_main['volume'][up_mask]
                down_x = df_main.index[~up_mask]
                down_y = df_main['volume'][~up_mask]

                # Downsample each series
                up_x_ds, up_y_ds = downsample_data(up_x, up_y)
                down_x_ds, down_y_ds = downsample_data(down_x, down_y)

                if len(up_x_ds) > 0:
                    fig.add_trace(go.Scattergl(x=up_x_ds, y=up_y_ds, mode='lines', name='Volume (Up)',
                                              line=dict(color='#2ECC71', width=0), fill='tozeroy',
                                              fillcolor='rgba(46,204,113,0.5)', showlegend=False), row=row_i, col=1)
                if len(down_x_ds) > 0:
                    fig.add_trace(go.Scattergl(x=down_x_ds, y=down_y_ds, mode='lines', name='Volume (Down)',
                                              line=dict(color='#E74C3C', width=0), fill='tozeroy',
                                              fillcolor='rgba(231,76,60,0.5)', showlegend=False), row=row_i, col=1)
            else:
                fig.add_trace(go.Bar(x=df_main.index, y=df_main['volume'], name='Volume', marker_color=vol_colors), row=row_i, col=1)
        else:
            x_ds, y_ds = downsample_data(df_main.index, df_main[key])
            trace = LineTrace(x_ds, y_ds, title, width=1.2, opacity=0.9)
            # Optimize hover for performance
            if '%' in title:
                trace.hovertemplate = f"{title}: %{{y:.2f}}%<extra></extra>"
            else:
                trace.hovertemplate = f"{title}: %{{y:.4f}}<extra></extra>"
            trace.hoverinfo = 'y' if use_gl else 'all'
    ...
    main_positions = add_markers_to_plotly(fig, df_m_for_markers, analysis_results, 'V1 (Main)', extra_targets=extra_targets, use_gl=use_gl)
    ...
    if df_var2 is not None and variant2_results is not None:
        ...
        v2_positions = add_markers_to_plotly(fig, df_v2_for_markers, variant2_results, 'V2 (Variant)', extra_targets=extra_targets, use_gl=use_gl)
        ...
        def _add_outline(dates_prices, name, color):
            ...
            # Preis
            price_outline = ScatterClass(x=list(dates), y=list(prices), mode='markers',
                                      name=name, marker=mk,
                                      legendgroup=f"Comparison|{name}",
                                      legendgrouptitle_text="Comparison",
                                      showlegend=True)
            price_outline.hovertemplate = f"{name}<br>Price: %{{y:.4f}}<extra></extra>"
            price_outline.hoverinfo = 'text' if use_gl else 'all'
            fig.add_trace(price_outline, row=1, col=1)

            # RSI
            rsi_outline = ScatterClass(x=list(dates), y=rsi_vals, mode='markers',
                                      name=name, marker=mk,
                                      legendgroup=f"Comparison|{name}",
                                      showlegend=False)
            rsi_outline.hovertemplate = f"{name}<br>RSI: %{{y:.1f}}<extra></extra>"
            rsi_outline.hoverinfo = 'text' if use_gl else 'all'
            fig.add_trace(rsi_outline, row=2, col=1)

            # MACD
            macd_outline = ScatterClass(x=list(dates), y=macd_vals_local, mode='markers',
                                      name=name, marker=mk,
                                      legendgroup=f"Comparison|{name}",
                                      showlegend=False)
            macd_outline.hovertemplate = f"{name}<br>MACD: %{{y:.6f}}<extra></extra>"
            macd_outline.hoverinfo = 'text' if use_gl else 'all'
            fig.add_trace(macd_outline, row=3, col=1)
    ...
    if df_var3 is not None and variant3_results is not None:
        ...
        v3_positions = add_markers_to_plotly(fig, df_v3_for_markers, variant3_results, 'V3 (Variant 3)', extra_targets=extra_targets, use_gl=use_gl)
    ...
    # Performance optimizations for layout
    layout_updates = {
        ...
        'hovermode': 'x' if not use_gl else 'closest',  # 'x' is faster for large datasets
        'hoverlabel': dict(
    ...
    # Add performance-specific layout options for large datasets
    if use_gl:
        layout_updates.update({
            'spikedistance': -1,  # Enable spike lines
            'hoverdistance': 10,  # Smaller hover distance for better performance
        })

    fig.update_layout(**layout_updates)
    # Optimize axis performance
    fig.update_yaxes(title_text="Price", row=1, col=1, autorange=True, rangemode='normal')

    # RSI axis with optimized settings
    fig.update_yaxes(title_text="RSI", autorange=True, rangemode='normal',
                     tickmode='auto', nticks=6, row=2, col=1)

    # MACD axis with optimized settings
    fig.update_yaxes(title_text="MACD", autorange=True, rangemode='normal',
                     tickmode='auto', nticks=5, row=3, col=1)

    # Set y-axis titles for extra panels with performance optimizations
    for idx, (title, key, _px) in enumerate(panels, start=1):
        fig.update_yaxes(title_text=title, row=3+idx, col=1, autorange=True, rangemode='normal',
                         tickmode='auto', nticks=4)

    # Optimize x-axis for performance
    fig.update_xaxes(showspikes=True, spikemode='across', spikethickness=1,
                     showgrid=False, row=1, col=1)  # Only show spikes on main chart
    ...
    try:
        ...
        # Build extended hover relayout args: optimized for performance
        extended_args = {
            'hovermode': 'x' if use_gl else 'x unified',  # 'x' is faster for large datasets
            'hoverlabel.namelength': -1,
            'spikedistance': 10 if use_gl else -1  # Smaller spike distance for performance
        }
        # Only enable spikes on main chart for performance
        extended_args['xaxis.showspikes'] = True
        extended_args['yaxis.showspikes'] = True
        extended_args['yaxis.spikemode'] = 'across'
        hover_buttons = [
    ...
    config = {
        ...
        'displayModeBar': True,  # Keep mode bar but optimize it
        'displaylogo': False,  # Hide Plotly logo
        'modeBarButtonsToRemove': ['lasso2d', 'select2d'],  # Keep pan2d, remove only lasso and select
        'modeBarButtonsToAdd': ['pan2d'],  # Explicitly add pan button
        'scrollZoom': True,  # Enable scroll zoom
        'editable': False,  # Disable editing for performance
        'staticPlot': False,  # Keep interactive
        'doubleClick': 'reset',  # Double-click to reset zoom
        'showTips': False,  # Disable tips for cleaner UI
    }

    # Add performance-specific config for large datasets
    if use_gl:
        config.update({
            'displayModeBar': 'hover',  # Show mode bar only on hover
            'modeBarButtonsToAdd': [],  # No additional buttons
        })

    # Write HTML with optimizations
    fig.write_html(
        out_html,
        config=config,
        include_plotlyjs='cdn',  # Use CDN for faster loading
        include_mathjax=False,  # Disable MathJax if not needed
        auto_open=False,  # Don't auto-open browser
        full_html=True,  # Generate complete HTML
        default_width='100%',  # Responsive width
        default_height='100%'  # Responsive height
    )

    print(f"\nInteractive plot saved to: {out_html}")
    print(f"Performance optimizations applied: GL={use_gl}, Downsample={downsample}")

    # Only show plot if not in headless environment
    try:
        fig.show(config=config)
    except Exception:
        print("Plot display skipped (headless environment)")
