...
- Explorer-Auswahl (--pick ODER kein --input): 1..n Dateien, merkt letztes Verzeichnis
"""
...
def pick_files_via_explorer() -> list[Path]:
    """Öffnet Windows-Explorer zur Auswahl 1..n Dateien (Parquet/CSV)."""
    if not TK_OK:
...
def add_markers_for_run(fig: go.Figure, res: pd.DataFrame, base_df: pd.DataFrame,
    ...
    """
    Farbe = Signaltyp: gen→blau (#1f77b4), neg→rot (#d62728). Form = symbol_for_set.
...
def run_for_file(input_path: Path, args, alphas: list[float], gammas: list[float]):
    ...
    basename = f"{input_path.stem}_{stamp}"

    if args.save_csv:
        csv_path = out_dir / f"overlay_counts_{basename}.csv"
        df_counts.to_csv(csv_path, index=False)
        print(f"[OK] CSV gespeichert: {csv_path}")

    if args.save_html:
        overlay_html = out_dir / f"overlay_plot_{basename}.html"
        heatmap_html = out_dir / f"overlay_heatmap_{basename}.html"
    ...
    if args.validate:
        ...
        val_csv = out_dir / f"validation_{basename}.csv"
        val_df.to_csv(val_csv, index=False)
        print(f"[OK] Validierungsreport: {val_csv}")

    # XLSX
    if args.save_xlsx:
        save_xlsx(out_dir, basename, df_counts, val_df, alphas, gammas)
...
def main():
    ...
    if args.pick or not args.input:
        chosen = pick_files_via_explorer()
        if not chosen:
            print("[INFO] Keine Dateien gewählt. Abbruch.")
            return
        input_paths = chosen
    else:
