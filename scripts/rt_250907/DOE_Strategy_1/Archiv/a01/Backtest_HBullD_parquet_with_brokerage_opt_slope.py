...
if __name__ == '__main__':
    ...
    if args.input_file:
        ...
        if not (os.path.isfile(input_file) and input_file.lower().endswith('.parquet')):
            raise FileNotFoundError(f"--file not found or not a parquet: {input_file}")
    ...
    # 3) Tkinter folder dialog
        ...
        # If still nothing, search recursively under CWD for the first directory with parquet
    ...
    # Vectorized Divg_Slope calculation (guard against divide-by-zero/NaN)
    with np.errstate(divide='ignore', invalid='ignore'):
        ...
        divg1 = 10000 * np.divide(np.abs(hb_ll_gen - hb_hl_gen), denom1, out=np.zeros_like(hb_ll_gen), where=(denom1 != 0))
        divg2 = 10000 * np.divide(np.abs(hb_ll_neg - hb_hl_neg), denom2, out=np.zeros_like(hb_ll_neg), where=(denom2 != 0))
        divg3 = 10000 * np.divide(np.abs(cb_ll_gen - cb_hl_gen), denom3, out=np.zeros_like(cb_hl_gen), where=(denom3 != 0))
        divg4 = 10000 * np.divide(np.abs(cb_ll_neg - cb_hl_neg), denom4, out=np.zeros_like(cb_hl_neg), where=(denom4 != 0))
    divg5 = divg3
