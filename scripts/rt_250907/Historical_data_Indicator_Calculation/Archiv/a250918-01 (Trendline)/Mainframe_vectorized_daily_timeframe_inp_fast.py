    ...
    try:
        ...
        try:
            ...
            if sys.platform == 'win32':
                ...
                # Fallback to legacy folder browser if needed
                if not selected:
...
_df_processed = None
_processed_parquet_path = None

def _pool_init(parquet_path):
    # Set BLAS threads low to avoid oversubscription
    os.environ.setdefault('OMP_NUM_THREADS', '1')
    os.environ.setdefault('MKL_NUM_THREADS', '1')
    os.environ.setdefault('OPENBLAS_NUM_THREADS', '1')
    os.environ.setdefault('NUMEXPR_MAX_THREADS', '1')
    global _processed_parquet_path, _df_processed
    _processed_parquet_path = parquet_path
    _df_processed = None

def _get_df_processed():
    global _df_processed
    if _df_processed is None:
        _df_processed = pd.read_parquet(_processed_parquet_path)
    return _df_processed

def process_i(i):
    # Slice from preprocessed full dataset (loaded once per worker)
    df = _get_df_processed().iloc[:i].copy()
...
def _sigint_handler(signum, frame):
    # mark interrupt; main loop will persist and pause
...
if __name__ == '__main__':
    ...
    start_i = 200
    # Read full CSV once (main process only) and expose via parquet to workers
    df_full = pd.read_csv(csv_file_path, header=0, parse_dates=['date'])
    # Run full indicator pipeline once on the entire dataset
    Initialize_RSI_EMA_MACD(df_full)
    Level_1_Max_Min(df_full)
    Candlestick_Type(df_full)
    CBullDivg_analysis(df_full, 0.1, 3.25)
    CBullDivg_x2_analysis(df_full, 0.1, 3.25)
    HBullDivg_analysis(df_full, 0.1, 3.25)
    CBearDivg_analysis(df_full, 0.1, 3.25)
    HBearDivg_analysis(df_full, 0.1, 3.25)
    df_full = calculate_support_levels(df_full, lookback_years=25, pivot_threshold=0.25)
    df_full = calc_TL_Up_Support(df_full, min_gap=20, adjacent_candles=10, exclude_end_points=7)
    df_full = calc_TL_Up_Resistance(df_full, min_gap=20, adjacent_candles=10, exclude_end_points=7)
    df_full = calc_TL_Down_Resistance(df_full, min_gap=20, adjacent_candles=10, exclude_end_points=7)
    df_full = calculate_golden_ratios(df_full)

    processed_parquet_path = os.path.join(output_dir, 'processed_df.parquet')
    df_full.to_parquet(processed_parquet_path, index=False, engine='pyarrow')
    len_df = len(df_full)
    del df_full
    end_i = len_df - 1
    total_all = max(0, end_i - start_i)

    # Load resume state and compute remaining work
    state_path, control_path = _state_paths(csv_file_path, output_dir)
    state = _load_state(state_path)
    # Clean completed outside range
    state['completed'] = set(i for i in state.get('completed', set()) if start_i <= i < end_i)
    # If no prior state, infer progress from existing output files (avoid double work)
    if not os.path.isfile(state_path):
        try:
            # Prefer sidecar JSONs with explicit indices
            ...
            for sc in sidecars:
                try:
                    ...
                    continue
            if not got_any:
                # Fallback heuristic based on parquet count
    ...
    if total == 0:
        ...
        with mp.Pool(processes=procs_count, initializer=_pool_init, initargs=(processed_parquet_path,)) as pool:
            try:
                for i_done in pool.imap_unordered(process_i, remaining_iter, chunksize=chunksize):
                    state['completed'].add(i_done)
                    completed += 1
                    # Periodically persist and check for pause/abort
                ...
                try:
                    pool.close()  # soft close; no new tasks submitted
                except Exception:
                    pass
                try:
                    pool.join()
    ...
    try:
        os.remove(processed_parquet_path)
    except Exception:
        pass
