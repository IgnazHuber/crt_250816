...
def is_invalid_trendline_vectorized(opens, highs, lows, closes, slope, y_intercept, n, start_idx, end_idx):
    """
    Vectorized check for invalid upward resistance trendlines using NumPy operations.
...
def calc_TL_Up_Resistance(df, min_gap, adjacent_candles, exclude_end_points):
    """
    Calculate upward resistance trendlines for the entire DataFrame up to the last row and add their coordinates and dates to the last row.
    If no trendlines are found, populate with dummy values (0 for prices, first/last dates for dates).
    Connects local highs with an upward slope, ensuring no candle bodies are above the trendline,
    no candles are entirely above, and up to one upper wick deviation is allowed.
    ...
    - DataFrame with additional columns for up to 7 trendlines:
      ['TL_U_Resistance_{i}_Start_Date', 'TL_U_Resistance_{i}_End_Date', 'TL_U_Resistance_{i}_Start_Price', 'TL_U_Resistance_{i}_End_Price']
    """
    ...
    # Filter pairs for upward trendlines (second high >= first high)
    ...
    for start_idx, start_high, end_idx, end_high, start_date, end_date in trendlines:
        ...
        if is_adjacent:
            # Replace if current trendline has lower highs (closer to price action)
