...
# Input selection deferred to main guard
...
def process_i(i):
    # Slice from original dataset, then compute indicators to match legacy output
    df = _get_df_full().iloc[:i].copy()

    # Run full indicator pipeline as original module
    ...
    # Save output (same behaviour as original script)
