...
# Ensure main process handles input selection


_df_full = None
_temp_parquet_path = None
...
def _pool_init(parquet_path):
    ...
    global _temp_parquet_path, _df_full
    _temp_parquet_path = parquet_path
    _df_full = None

def _get_df_full():
    global _df_full
    if _df_full is None:
        _df_full = pd.read_parquet(_temp_parquet_path)
    return _df_full

def process_i(i):
    # Slice from shared full dataset (loaded once per worker)
    df = _get_df_full().head(i).copy()
...
if __name__ == '__main__':
    ...
    temp_parquet_path = os.path.join(output_dir, 'temp_df.parquet')
    ...
    if total == 0:
        ...
        with mp.Pool(processes=procs_count, initializer=_pool_init, initargs=(temp_parquet_path,)) as pool:
            try:
