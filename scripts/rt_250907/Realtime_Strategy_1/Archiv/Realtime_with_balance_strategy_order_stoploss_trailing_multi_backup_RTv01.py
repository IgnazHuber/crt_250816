...
def log_data_summary(df, symbol, interval, stage, folder=None):
    """Log concise info to confirm data presence/health at key stages.

    - Logs row count, time span, and basic monotonicity check.
    - Optionally writes a short marker line to a per-folder `last_update.txt`.
    """
    try:
        if df is None:
            logger.warning(f"[{stage}] {symbol} {interval}: DataFrame is None")
            return False
        if df.empty:
            logger.warning(f"[{stage}] {symbol} {interval}: DataFrame is empty")
            return False
        required = ['timestamp', 'close']
        missing = [c for c in required if c not in df.columns]
        if missing:
            logger.warning(f"[{stage}] {symbol} {interval}: Missing columns: {missing}")
            return False
        # Safe conversion for checks
        ts = pd.to_datetime(df['timestamp'], errors='coerce', utc=True)
        valid_ts = ts.notna().sum()
        if valid_ts == 0:
            logger.warning(f"[{stage}] {symbol} {interval}: All timestamps invalid after parse")
            return False
        # Check monotonic non-decreasing
        is_mono = (ts.dropna().diff().dropna() >= pd.Timedelta(0)).all()
        span = f"{ts.iloc[0]} -> {ts.iloc[-1]}"
        logger.info(
            f"[{stage}] {symbol} {interval}: rows={len(df)}, valid_ts={valid_ts}, monotonic={is_mono}, span={span}"
        )
        if folder:
            try:
                os.makedirs(folder, exist_ok=True)
                with open(os.path.join(folder, 'last_update.txt'), 'w', encoding='utf-8') as f:
                    f.write(f"{datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S')} | "
                            f"{stage} | rows={len(df)} | span={span}\n")
            except Exception as e:
                logger.debug(f"[{stage}] Could not write last_update.txt for {symbol} {interval}: {e}")
        return True
    except Exception as e:
        logger.warning(f"[{stage}] {symbol} {interval}: Exception during data summary: {e}")
        return False

def save_parquet(df, folder):
    """Save DataFrame to parquet if possible, otherwise fallback to CSV.

    The file is named with the last timestamp. If parquet save fails due to
    missing engine or any exception, a CSV is written instead. Downstream
    code is updated to read either parquet or CSV transparently.
    """
    ...
    file_timestamp = str(last_timestamp).replace(':', '-').replace(' ', '_')
    parquet_path = os.path.join(folder, f"historical_data_{file_timestamp}.parquet")
    try:
        # Prefer pyarrow if available, otherwise let pandas pick
        df.to_parquet(parquet_path, index=False)
        print(f"Saved parquet file: {parquet_path} with {len(df)} rows")
        return parquet_path
    except Exception as e:
        # Fall back to CSV if parquet engine is unavailable
        csv_path = os.path.join(folder, f"historical_data_{file_timestamp}.csv")
        try:
            df.to_csv(csv_path, index=False)
            print(
                f"Parquet save failed ({e}); saved CSV instead: {csv_path} with {len(df)} rows"
            )
            return csv_path
        except Exception as e2:
            print(f"Failed to save data to both parquet and CSV: {e2}")
            return None


def list_data_files(folder):
    """List saved data files (parquet or CSV) matching historical_data_* pattern."""
    if not os.path.isdir(folder):
        return []
    return [
        f for f in os.listdir(folder)
        if f.startswith('historical_data_') and (f.endswith('.parquet') or f.endswith('.csv'))
    ]


def read_data_file(folder, filename):
    """Read a saved data file (parquet or CSV) into a DataFrame."""
    path = os.path.join(folder, filename)
    if filename.endswith('.parquet'):
        try:
            return pd.read_parquet(path)
        except Exception as e:
            # As a last resort, try reading as CSV if parquet engine missing but file is CSV-in-parquet
            try:
...
def process_and_save_df(df, symbol, interval, folder, pending_order, pending_order_file, is_realtime=False):
    ...
    Robust against indicator/strategy failures: if indicators fail, fall back to raw df; if strategy fails, continue
    with indicator-only df. Always attempt to save at the end so data files exist.
    """
    df_raw = df.copy()
    df = compute_indicators(df, symbol, interval)
    if df is None:
        logger.warning(f"compute_indicators failed for {symbol} {interval}; saving raw data without indicators.")
        df = df_raw
    try:
        df = strategy_1(df, symbol, interval)
    except Exception as e:
        logger.warning(f"strategy_1 failed for {symbol} {interval}: {e}. Proceeding without strategy columns.")
        if pending_order:
        ...
        try:
            ...
            logger.warning(f"apply_trailing_stoploss failed for {symbol} {interval}: {e}. Skipping trailing stop.")
        # Check if this is the initial run
        existing_files = list_data_files(folder)
        ...
        save_path = save_parquet(df, folder)
        if save_path:
            logger.info(f"Saved data for {symbol} {interval} to {save_path}")
    else:
        potential_order = None
...
def execute_order(potential_order):
    ...
    if action == 'buy':
        ...
        if not existing_files:
            logger.error(f"No data files found for {symbol} {interval}, cannot execute sell order")
            return
        latest_file = max(existing_files, key=lambda f: os.path.getctime(os.path.join(folder, f)))
        df = read_data_file(folder, latest_file)
        log_data_summary(df, symbol, interval, stage='sell_load_latest', folder=folder)
...
def archive_old_files(folders, frequency):
    ...
    delta = get_archive_delta(frequency)
    archive_base = "archives"
    now = datetime.now(timezone.utc)
    cutoff = now - delta

    for folder in folders:
        os.makedirs(os.path.join(archive_base, folder), exist_ok=True)
        files_to_archive = []
        for file in os.listdir(folder):
            if file.startswith('historical_data') and (file.endswith('.parquet') or file.endswith('.csv')):
        ...
        if files_to_archive:
            archive_name = f"historical_data_{folder}_{now.strftime('%Y%m%d_%H%M')}.7z"
            archive_path = os.path.join(archive_base, folder, archive_name)
            cmd = ['7z', 'a', '-t7z', archive_path] + files_to_archive
            result = subprocess.run(cmd, capture_output=True, text=True)
            if result.returncode == 0:
                logger.info(f"Archived {len(files_to_archive)} files for {folder} to {archive_path}")
                for file_path in files_to_archive:
                    os.remove(file_path)
                logger.info(f"Removed archived files from {folder}")
            else:
...
def update_data(symbol, interval):
    ...
    if not existing_files:
        print(f"No data files found for {symbol} {interval}. Fetching last 1000 candles.")
        ...
        if not df.empty:
            ...
            # Ensure only complete candles (exclude last if incomplete)
            df['ts_dt'] = pd.to_datetime(df['timestamp'], utc=True)
            df = df[df['ts_dt'] + interval_delta <= now_dt].copy().drop('ts_dt', axis=1)
            log_data_summary(df, symbol, interval, stage='initial_fetch_filtered', folder=folder)
            fetch_balances_and_set(df, symbol)
            df, potential_order = process_and_save_df(df, symbol, interval, folder, pending_order, pending_order_file, is_realtime=False)
            return potential_order
        logger.warning(f"No data returned for {symbol} {interval} from {start_time} to {current_time_str}.")
        return None

    # Load latest file and get last timestamp
    latest_file = max(existing_files, key=lambda f: os.path.getctime(os.path.join(folder, f)))
    df = read_data_file(folder, latest_file)
    log_data_summary(df, symbol, interval, stage='load_latest_file', folder=folder)
    if df.empty:
        print(f"Latest data file for {symbol} {interval} is empty. Fetching last 1000 candles.")
        start_time = (now_dt - timedelta(days=200)).strftime('%Y-%m-%d %H:%M:%S')
        df = get_candlestick_data(session_demo, symbol, start_time, current_time_str, interval)
        if not df.empty:
            df['timestamp'] = df['timestamp'].apply(format_date)
            df['ts_dt'] = pd.to_datetime(df['timestamp'], utc=True)
            df = df[df['ts_dt'] + interval_delta <= now_dt].copy().drop('ts_dt', axis=1)
            log_data_summary(df, symbol, interval, stage='empty_latest_refetch_filtered', folder=folder)
            fetch_balances_and_set(df, symbol)
            df, potential_order = process_and_save_df(df, symbol, interval, folder, pending_order, pending_order_file, is_realtime=False)
            return potential_order
        logger.warning(f"No data returned for {symbol} {interval} from {start_time} to {current_time_str}.")
    ...
    last_timestamp_str = df.iloc[-1]['timestamp']
    ...
    potential_order = None
    while new_start_time < now_dt:
        chunk_end_time = min(new_start_time + timedelta(seconds=interval_seconds * 999), now_dt)
        ...
        if not new_df.empty:
            ...
            new_df['ts_dt'] = pd.to_datetime(new_df['timestamp'], utc=True)
            new_df = new_df[new_df['ts_dt'] + interval_delta <= now_dt].copy().drop('ts_dt', axis=1)
            new_df = new_df[~new_df['timestamp'].isin(df['timestamp'])]
            if not new_df.empty:
                log_data_summary(new_df, symbol, interval, stage='chunk_fetch_filtered', folder=folder)
                ...
                for idx in range(len(new_df)):
                    ...
                    is_realtime = (idx == len(new_df) - 1)
                    updated_df, po = process_and_save_df(updated_df, symbol, interval, folder,
...
def wait_until_next_update(intervals):
    ...
    now = datetime.now(timezone.utc)  # Use UTC directly
    min_interval = min([get_interval_delta(interval) for interval in intervals], default=timedelta(minutes=15))
    min_interval_minutes = min_interval.total_seconds() / 60
    # Calculate minutes since midnight in UTC
    minutes_since_midnight = now.hour * 60 + now.minute
    # Find the next candle boundary (e.g., 00:15, 00:30, 00:45 for 15-minute interval)
    next_boundary_minutes = math.ceil(minutes_since_midnight / min_interval_minutes) * min_interval_minutes
    next_update = now.replace(hour=int(next_boundary_minutes // 60), minute=int(next_boundary_minutes % 60), second=0, microsecond=0)
    if next_update <= now:
...
def main():
    ...
    # Initialize LAST_ARCHIVE_TIME if not set
